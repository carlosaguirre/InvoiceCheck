<?php
    ...
    $lstPfx="<!-- 1stMsg -->";
    $lpLen=strlen($lstPfx);
    $_esAdministrador=$_esSistemas=$_esDesarrollo=$_esPruebas=$_esCompras=$_esProveedor=false;
    if ($habilitado) {
//        echo "<!-- ACTIVE -->\n";
        include "configuracion/inicializacion.php";
        include "configuracion/login.php";
        
//        $mensajeImportante=MENSAJE_VERSION40_0;
    ...
    //clog2("USER: ".json_encode(getUser()));
            ...
            //$_esSistemas = validaPerfil("Sistemas");
            //$_esDesarrollo = getUser()->nombre==="admin";
            //$_esPruebas = in_array(getUser()->nombre, ["admin","sistemas","sistemas1","sistemas2","test","test1","test2","test3"]);
            //$_esCompras = validaPerfil("Compras");
            //$_esProveedor = validaPerfil("Proveedor");
            if ($modoActualizacion) {
                $menu_accion="Actualizacion";
//                echo "<!-- UPDATING -->\n";
            } else {
                if ($_esPruebas) {
...
//                    echo "<!-- TESTING -->\n";
                ...
                if ($modoPruebas && !$_esPruebas) $menu_accion="Actualizacion";
                else if(getUser()->cambiaClave) $menu_accion="CambiaClave";
            }
        }
    } else {
//        echo "<!-- INACTIVE -->\n";
...
//ini_set('display_startup_errors', 1);
//error_reporting(E_ALL);
        include "configuracion/inicializacionSinBD.php";
        include "configuracion/login.php";
        $hasUser=hasUser();
        if ($hasUser) $menu_accion="Actualizacion";
    }
    if (!isset($menu_accion[0])) {
        ...
        } else doclog("OTHER: no menu action: '$menu_accion'","action");

        if (isset($_SESSION["tmp"])) {
            if ($_SESSION["tmp"]==="loggedin") {
                if((getUser()->banderas & 2)>0) { // Tiene bandera 2
                    ...
                    $usrObj->saveRecord(["id"=>getUser()->id,"banderas"=>new DBExpression("banderas&253")]); // Borrar bandera 2
                }
            }
            unset($_SESSION["tmp"]);
        }
        if ((($_esSistemas??false)||($_esAdministrador??false)) && (!isset($_POST["menu_accion"])||$_POST["menu_accion"]==="Inicio")) {
            ...
            if (!isset($invObj)) {
                ...
                $invObj=new Facturas();
                //doclog("new Facturas","test",["tablename"=>$invObj->tablename]);
            }
        ...
        if (isset(getUser()->proveedor) && !isset($_REQUEST["logout"])) {
            if (getUser()->proveedor->status==="inactivo") $menu_accion="Inactivo";
                ...
                /*$noCCPData=$invObj->getData("f.codigoProveedor=\"".getUser()->proveedor->codigo."\" and f.tipoComprobante=\"i\" and f.metodoDePago=\"PPD\" and (f.idReciboPago is null or f.statusReciboPago is null or c.idCPago is null or f.statusReciboPago<1 or f.saldoReciboPago>0) and f.statusn between 32 and 127 and (p.statusn is null or p.statusn<127) and (g.statusn is null or g.statusn<127) and (f.fechaPago is null or f.fechaPago>\"2018-09-01 00:00:00\")", 0,
                    ...
                    "f left join dpagos d on f.id=d.idFactura left join cpagos c on d.idppago=c.id left join facturas p on c.idCPago=p.id left join facturas g on f.idReciboPago=g.id");
                */
                $noCCPData=$invObj->getDataFromTemp("f.codigoProveedor=\"".getUser()->proveedor->codigo."\" and f.tipoComprobante=\"i\" and f.metodoDePago=\"PPD\" and f.statusn between 32 and 127 and (g.id is null or g.statusn between 1 and 127) and (f.fechaPago is null or f.fechaPago>\"2018-09-01 00:00:00\")", 
...
// group_concat([<c|d>pagoField] order by d.saldoInsoluto asc) => group_concat([<c|d>pagoField] order by d.idFactura desc, c.idCPago desc)
                ...
                if (isset($noCCPData[0])) {
                    ...
                    $eacp=trim(str_replace("-","",getUser()->proveedor->codigo));

                    $nln=0;
                    $fxRP=[]; $fxI=[];
                    $frcFx=false;
// f.id,f.ubicacion,f.uuid,f.serie,f.folio,f.fechaFactura,f.total,f.moneda,f.nombreInterno,f.nombreInternoPDF,f.ea,f.version,f.idReciboPago,f.statusReciboPago,f.saldoReciboPago,f.statusn,c.idCPago,c.fechaPago,d.numParcialidad,d.saldoInsoluto,p.statusn cpStatusN,g.statusn rpStatusN
                    foreach ($noCCPData as $idx => $crow) {
                        $idFactura="".$crow["id"]??"";
                        if (isset($fxI[$idFactura])) {
                            $d2d++;
                            doclog("Index noCCPData","pagosDup",["usuario"=>getUser()->nombre,"codigo"=>getUser()->proveedor->codigo,"idx"=>$idx,"idFactura"=>$idFactura,"invNoCPData"=>$crow,"saved"=>$fxI[$idFactura]]);
                            continue;
                        }
                        $pfolio=$crow["folio"]??"";
...
//                        echo "<!-- FACTURA='$idFactura', FOLIO='$pfolio', RPAGO='$idReciboPago'".
                            (isset($idReciboPago[0])?"1":"0").
                            ", CPAGO='$idCPago'".
                            (isset($idCPago[0])?"1":"0").
                            (($idReciboPago==$idCPago)?"1":"0").
                            ", STATUSRP='$statusReciboPago'".
                            (empty($statusReciboPago[0])?"1":"0").
                            ", CPSTTN='$cpStatusN'$cpTest, FxSttRP='$fixSttRP' ".
                            "-->\n";
                        if (!isset($idReciboPago[0])) {
                            ...
                            if (isset($idCPago[0])) {
                                ...
                                if ($invObj->saveRecord($fxArrFld)) {
                                    ...
                                    if ($sttRP==="LIQUIDADO") {
                                        $d2d++;
                                        doclog("Index noCCPData FIXED 1","pagos",["usuario"=>getUser()->nombre,"codigo"=>getUser()->proveedor->codigo,"idx"=>$idx,"idFactura"=>$idFactura,"invNoCPData"=>$crow,"saved"=>$fxArrFld]);
                                        continue;
                                    }
                                } else if (empty(DBi::getErrno()) && $sttRP==="LIQUIDADO") {
                                    continue;
                                } else {
                                    doclog("Index noCCPData FAIL SAVE 1","pagosErr",["usuario"=>getUser()->nombre,"codigo"=>getUser()->proveedor->codigo,"idFactura"=>$idFactura,"preSaved"=>($fxI[$idFactura]??""),"invNoCPData"=>$crow,"query"=>$query,"errcode"=>DBi::getErrno(),"errmsg"=>DBi::getError(),"dbiErrors"=>DBi::$errors,"invErrors"=>$invObj->errors]);
                                }
                            }
                            ...
                            if ($invObj->saveRecord($fxArrFld)) {
                                ...
                                if ($sttRP==="LIQUIDADO") {
                                    $d2d++;
                                    doclog("Index noCCPData FIXED 2","pagos",["usuario"=>getUser()->nombre,"codigo"=>getUser()->proveedor->codigo,"idx"=>$idx,"idFactura"=>$idFactura,"invNoCPData"=>$crow,"saved"=>$fxArrFld]);
                                    continue;
                                }
                            } else if (empty(DBi::getErrno()) && $sttRP==="LIQUIDADO") {
                                continue;
                            } else doclog("Index noCCPData FAIL SAVE 2","pagosErr",["usuario"=>getUser()->nombre,"codigo"=>getUser()->proveedor->codigo,"idFactura"=>$idFactura,"preSaved"=>$fxI[$idFactura],"foundInvNoCPData"=>$crow,"query"=>$query,"errcode"=>DBi::getErrno(),"errmsg"=>DBi::getError(),"dbiErrors"=>DBi::$errors,"invErrors"=>$invObj->errors]);
                ...
                if (getUser()->proveedor->status==="actualizar") {
//                    echo "<!-- PROVEEDOR DESACTUALIZADO -->";
                    $menu_accion="ActualizaCuentaBancaria";
                    $errorTitle="Actualizar Datos";
                    $errorMessage.=$prefixOnce.MENSAJE_ACTUALIZAR1;
                    $prefixOnce="";
                } else if (getUser()->proveedor->verificado<0) {
//                    echo "<!-- CUENTA RECHAZADA -->";
                    $menu_accion="ActualizaCuentaBancaria";
                    $errorTitle="Actualizar Datos";
                    $errorMessage.=$prefixOnce.MENSAJE_ACTUALIZAR2;
                    $prefixOnce="";
                } else if (getUser()->proveedor->cumplido<0) {
                    $menu_accion="ActualizaCuentaBancaria";
                    if (getUser()->proveedor->cumplido<-1) {
//                        echo "<!-- OPINION RECHAZADA -->";
                        $errorTitle="Actualizar Datos";
                        $errorMessage.=$prefixOnce.MENSAJE_ACTUALIZAR3;
                        $prefixOnce="";
                    } else if (empty(getUser()->proveedor->opinion)) {
//                        echo "<!-- FALTA OPINION -->";
                        $errorTitle="Actualizar Datos";
                        $errorMessage.=$prefixOnce.MENSAJE_ACTUALIZAR4;
                        $prefixOnce="";
                    } else {
//                        echo "<!-- OPINION VENCIDA -->";
                        $errorTitle="Actualizar Datos";
                ...
                if (getUser()->proveedor->status==="bloqueado"/* && isset($_SESSION['MENSAJE_INICIAL'])*/) {
//                    echo "<!-- BLOQUEADO -->";
                ...
                if (isset(AVISO["PROVEEDOR"][0])) { // TRES VECES
                    $hasFirstWarning=(getUser()->banderas & 8)>0;
                    $hasSecondWarning=(getUser()->banderas & 16)>0;
                    $hasThirdWarning=(getUser()->banderas & 32)>0;
                    if (!$hasThirdWarning && !$hasSecondWarning && !$hasFirstWarning) {
                        ...
                        $usrObj->saveRecord(["id"=>getUser()->id,"banderas"=>new DBExpression("banderas&253")]);
                    }
                }
                if (isset(MENSAJE["PROVEEDOR"][0])) { // UNA VEZ
                    $hasBeenNoticed=(getUser()->banderas & 4)>0;
                    if (!$hasBeenNoticed) {
    ...
    if (hasUser()&&getUser()->nombre==="viajero"&&(!isset($menu_accion[0])||$menu_accion==="Inicio"))
        $menu_accion="Viajero";
    ...
    switch ($menu_accion) {
        ...
        case "ListaSolPago":
            ...
            $hasStyleAction  = true;
            $hasScriptAction = true;
            //$templateSuffix = ($_esPruebas||getUser()->nombre==="jlobaton")?"_nxt":"";
            break;
        case "Logs":
            $urlAction = "logs";
            $hasScriptAction = true;
