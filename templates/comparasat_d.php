<?php
$autorizado=hasUser()&&(validaPerfil("Administrador")||validaPerfil("Sistemas")||validaPerfil("Compara Clientes"));
if (!$autorizado) {
    if (hasUser()) {
...
clog2ini("templates.comparasat_d");
clog1seq(1);
...
$query="SELECT DISTINCT tipoComprobante tipo FROM comparasat UNION SELECT DISTINCT tipoComprobante tipo FROM comparaavance";
$result = DBi::query($query);
$tipoMap=[""=>"TODOS"];
while ($row = $result->fetch_assoc()) {
...
?>
            <div id="areaTemplate">
            <h1 class="nowrap">Comparar CFDI de Clientes</h1>
...
<?php
clog1seq(-1);
clog2end("templates.comparasat_d");
