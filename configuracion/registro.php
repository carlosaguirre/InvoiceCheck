...
$consultaProv = consultaValida("Proveedor")||$_esSistemas;
$modificaProv = modificacionValida("Proveedor")||$_esSistemas;
$validaBanco = validaPerfil("Valida Bancarias")||$_esSistemas;
$validaOpinion = validaPerfil("Valida Opinion")||$_esSistemas;
$consultaMasiva = validaPerfil("Consulta Masiva Prv")||$_esSistemas;
$bloqueaProv = validaPerfil("BloquearPrv")||$_esSistemas;
$contaProv = validaPerfil("ContaProv")||$_esSistemas;
$consultaObjImp = consultaValida("Objeto Impuesto")||$_esSistemas;
$modificaObjImp = modificacionValida("Objeto Impuesto")||$_esSistemas;
$consultaPayTaxChk = consultaValida("CPago Impuestos")||$_esSistemas;
$modificaPayTaxChk = modificacionValida("CPago Impuestos")||$_esSistemas;
$consultaDefCvPrdSrv = consultaValida("ConceptoSindefinir")||$_esSistemas;
$modificaDefCvPrdSrv = modificacionValida("ConceptoSindefinir")||$_esSistemas;
$consultaTiposProveedor = validaPerfil(["TiposProveedor","ComercioExterior Monitor","ComercioExterior Control","ComercioExterior Admin","Direccion"])||$_esSistemas;
if (!$consultaProv&&!$consultaObjImp&&!$consultaPayTaxChk&&!$consultaDefCvPrdSrv) {
    setcookie("menu_accion", "", time() - 3600);
    setcookie("menu_accion", "", time() - 3600, "/invoice");
    header("Location: /".$_project_name."/");
    die("Redirecting to /".$_project_name."/\n<script>window.location.href = window.location.href+'?menu_accion=Inicio';</script>");
}
clog2ini("configuracion.registro");
clog1seq(1);

$soloLectura=(($_POST["view_mode"]??"")==="readonly");
$idProveedor=$_POST["prov_id"]??"";
$idUsuario=$_POST["user_id"]??"";
$codigoProveedor=strtoupper(trim($_POST["prov_code"]??""));
$razonProveedor=strtoupper(trim($_POST["prov_field"]??""));
$rfcProveedor=strtoupper(trim($_POST["prov_rfc"]??""));
$taxIdProveedor=trim($_POST["prov_taxid"]??"");
$tipoProveedor=+trim($_POST["prov_tipo"]??"0");
$emailUser=strtolower(trim($_POST["user_email"]??""));
$creditDays=trim($_POST["prov_credit"]??"");
$paymentCode=trim($_POST["prov_paym"]??"");
$bankName=trim($_POST["prov_bank"]??"");
$bankRfc=trim($_POST["prov_bankrfc"]??"");
$bankAccount=trim($_POST["prov_account"]??"");
$zonaProveedor=strtoupper(trim($_POST["prov_zone"]??""));
$textProveedor=$_POST["prov_text"]??"";
$statusProveedor=strtolower(trim($_POST["prov_status"]??""));
$accVerified=trim($_POST["acc_verified"]??"");
$opinionFulfilled=trim($_POST["opinion_fulfilled"]??"");
$opinionCreated=trim($_POST["opinion_created"]??"");
$opinionExpired=trim($_POST["opinion_expired"]??"");
$esServicio=in_array($_POST["esServicio"]??"", ["1","on"], true);
$conCodgEnDesc=in_array($_POST["conCodgEnDesc"]??"", ["1","on"], true);
$reqObjImp=in_array($_POST["reqObjImp"]??"", ["1","on"], true);
$reqPayTaxChk=in_array($_POST["reqPayTaxChk"]??"", ["1","on"], true);
$reqDefCvPrdSrv=in_array($_POST["reqDefCvPrdSrv"]??"", ["1","on"], true);
$referencias=$_POST["referencia"]??[];
$tipoComercial=in_array($_POST["esTComer"]??"", ["1","on"], true);
$tipoAduanal=in_array($_POST["esTAduan"]??"", ["1","on"], true);
$tipoTraslado=in_array($_POST["esTTrasl"]??"", ["1","on"], true);
$tipoLogistica=in_array($_POST["esTLogis"]??"", ["1","on"], true);

$beginHidden="beginHidden";
if (isset($_POST["prov_return"][0])) {
...
} else if (isset($_POST["prov_submit"])) {
    $doSavePrv=false;
    $doSaveUsr=false;
    $fldarr = [];
    $ufldarr = [];
    ...
    if (empty($codigoProveedor)) {
        $errorMessage .= $nvPrfo."Es necesario indicar el <b>C&oacute;digo</b> de proveedor.";
    ...
    if (empty($razonProveedor)) {
        $errorMessage .= $nvPrfo."Debe capturar la <b>Raz&oacute;n Social</b>.";
        if (!isset($focusId)) $focusId="prov_field";
        $nvPrfo="<br>";
    }
    if ($tipoProveedor<2) {
        if (empty($rfcProveedor)) {
            $errorMessage .= $nvPrfo."Se requiere que especifique su <b>RFC</b>.";
            if (!isset($focusId)) $focusId="prov_rfc";
            $nvPrfo="<br>";
        } else {
            require_once "clases/CatLista69B.php";
            if (CatLista69B::estaMarcado($rfcProveedor)) {
                $errorMessage .= $nvPrfo."El proveedor se encuentra en el listado 69-B por lo que no se autoriza su registro al portal.";
                $nvPrfo="<br>";
            }
        }
        if (empty($emailUser)) {
            if ($statusProveedor==="activo") {
                $errorMessage .= $nvPrfo."El <b>Correo Electr&oacute;nico</b> es requisito para env&iacute;o de notificaciones cuando status es <b>ACTIVO</b>";
        ...
        if (empty($taxIdProveedor)) {
            $errorMessage .= $nvPrfo."Se requiere que especifique su <b>Tax Id</b>.";
            if (!isset($focusId)) $focusId="prov_taxid";
            $nvPrfo="<br>";
        }
    }
    if (empty($creditDays) && $creditDays!=="0") {
        $errorMessage .= $nvPrfo."Debe indicar el número de <b>Días de crédito</b> para calcular la fecha de vencimiento de pago.";
        if (!isset($focusId)) $focusId="prov_credit";
        $nvPrfo="<br>";
    } 
    if (empty($bankName) && $statusProveedor==="activo") {
        $errorMessage.=$nvPrfo."Debe indicar el nombre del <b>Banco</b> relacionado con el pago cuando status es <b>ACTIVO</b>";
        if (!isset($focusId)) $focusId="prov_bank";
        $nvPrfo="<br>";
    }
    if (empty($bankRfc)&&$statusProveedor==="activo") {
        $errorMessage .= $nvPrfo."Debe indicar el <b>RFC del Banco</b> relacionado con el pago cuando status es <b>ACTIVO</b>";
        if (!isset($focusId)) $focusId="prov_bankrfc";
        $nvPrfo="<br>";
    }
    ...
    if (empty($errorMessage)) {
        if($esNuevoPrv && !validaFormatoCodigo($codigoProveedor, $razonProveedor)) {
    ...
    if ($esNuevoPrv) {
        ...
        list($dbcode,$dbrazsoc,$dbrfc,$dbnacional,$dbtipo,$dbbanco,$dbrfcbanco,$dbcuenta,$dbref1,$dbref2,$dbedocta,$dbctapg,$dbverificado,$dbopinion,$dbcumplido,$dbsatpg,$dbgenerado,$dbvencido,$dbcredito,$dbformapago,$dbzona,$dbstatus,$dbservicio,$dbcodgdesc,$dbobjimp,$dbRqPyTxChk,$dbRqDfCvPS) = explode("|",$prvObj->getValue("id",$idProveedor,"codigo,razonSocial,rfc,nacional,tipo,banco,rfcbanco,cuenta,referencia1,referencia2,edocta,numpagEdoCta,verificado,opinion,cumplido,numpagOpinion,generaopinion,venceopinion,credito,codigoFormaPago,zona,status,esServicio,conCodgEnDesc,reqObjImp,reqPayTaxChk,reqDefCvPrdSrv")); // 
        if ($dbcode!==$codigoProveedor && $prvObj->exists("codigo='$codigoProveedor'")) {
            ...
            if (!isset($focusId)) $focusId="prov_code";
            $nvPrfo="<br>";
        } else if (!$bloqueaProv && !empty($statusProveedor) && ($dbstatus==="bloqueado" || $dbstatus==="inactivo") && $statusProveedor!==$dbstatus) {
            $errorMessage .= $nvPrfo."Proveedor <b>".mb_strtoupper($dbstatus)."</b>. Consulte a Direcci&oacute;n por m&aacute;s informaci&oacute;n.";
            $nvPrfo="<br>";
        } else if ($dbstatus==="eliminado") {
            $errorMessage .= $nvPrfo."El proveedor fue <b>ELIMINADO</b> del sistema.";
            $nvPrfo="<br>";
        }
    }
    ...
    if (empty($errorMessage)) {
        ...
        if ($modificaProv&&!$soloLectura) {
            if(!$tipoComercial&&($esNuevoPrv||($dbtipo&Proveedores::TIPO_COMERCIAL)>0)) {
                $doSavePrv=true;
                if (isset($dbtipo)) {
                    $dbtipo-=Proveedores::TIPO_COMERCIAL;
                    $fldarr["tipo"]=$dbtipo;
                } else $fldarr["tipo"]=0;
            } else if ($tipoComercial&&!$esNuevoPrv&&($dbtipo&Proveedores::TIPO_COMERCIAL)==0) {
                $doSavePrv=true;
                $dbtipo+=Proveedores::TIPO_COMERCIAL;
                $fldarr["tipo"]=$dbtipo;
            }
            if($tipoAduanal&&($esNuevoPrv||($dbtipo&Proveedores::TIPO_ADUANA)==0)) {
                $doSavePrv=true;
                if (isset($dbtipo)) {
                    $dbtipo+=Proveedores::TIPO_ADUANA;
                    $fldarr["tipo"]=$dbtipo;
                } else {
                    if (!$fldarr["tipo"]) $fldarr["tipo"]=Proveedores::TIPO_COMERCIAL;
                    $fldarr["tipo"]+=Proveedores::TIPO_ADUANA;
                }
            } else if (!$tipoAduanal&&!$esNuevoPrv&&($dbtipo&Proveedores::TIPO_ADUANA)>0) {
                $doSavePrv=true;
                $dbtipo-=Proveedores::TIPO_ADUANA;
                $fldarr["tipo"]=$dbtipo;
            }
            if($tipoTraslado&&($esNuevoPrv||($dbtipo&Proveedores::TIPO_FLETE)==0)) {
                $doSavePrv=true;
                if (isset($dbtipo)) {
                    $dbtipo+=Proveedores::TIPO_FLETE;
                    $fldarr["tipo"]=$dbtipo;
                } else {
                    if (!$fldarr["tipo"]) $fldarr["tipo"]=Proveedores::TIPO_COMERCIAL;
                    $fldarr["tipo"]+=Proveedores::TIPO_FLETE;
                }
            } else if (!$tipoTraslado&&!$esNuevoPrv&&($dbtipo&Proveedores::TIPO_FLETE)>0) {
                $doSavePrv=true;
                $dbtipo-=Proveedores::TIPO_FLETE;
                $fldarr["tipo"]=$dbtipo;
            }
            if($tipoLogistica&&($esNuevoPrv||($dbtipo&Proveedores::TIPO_LOGISTICA)==0)) {
                $doSavePrv=true;
                if (isset($dbtipo)) {
                    $dbtipo+=Proveedores::TIPO_LOGISTICA;
                    $fldarr["tipo"]=$dbtipo;
                } else {
                    if (!$fldarr["tipo"]) $fldarr["tipo"]=Proveedores::TIPO_COMERCIAL;
                    $fldarr["tipo"]+=Proveedores::TIPO_LOGISTICA;
                }
            } else if (!$tipoLogistica&&!$esNuevoPrv&&($dbtipo&Proveedores::TIPO_LOGISTICA)>0) {
                $doSavePrv=true;
                $dbtipo-=Proveedores::TIPO_LOGISTICA;
                $fldarr["tipo"]=$dbtipo;
            }
            ...
            if ($modificaObjImp&&$reqObjImp&&($esNuevoPrv||$dbobjimp!=="1")) {
                $doSavePrv=true;
                $fldarr["reqObjImp"]="1";
            } else if ($modificaObjImp&&!$esNuevoPrv&&!$reqObjImp&&$dbobjimp==="1") {
                $doSavePrv=true;
                $fldarr["reqObjImp"]="0";
            }
            if ($modificaPayTaxChk&&$reqPayTaxChk&&($esNuevoPrv||$dbRqPyTxChk!=="1")) {
                $doSavePrv=true;
                $fldarr["reqPayTaxChk"]="1";
            } else if ($modificaPayTaxChk&&!$esNuevoPrv&&!$reqPayTaxChk&&$dbRqPyTxChk==="1") {
                $doSavePrv=true;
                $fldarr["reqPayTaxChk"]="0";
            }
            if ($modificaDefCvPrdSrv&&$reqDefCvPrdSrv&&($esNuevoPrv||$dbRqDfCvPS!=="1")) {
                $doSavePrv=true;
                $fldarr["reqDefCvPrdSrv"]="1";
            } else if ($modificaDefCvPrdSrv&&!$esNuevoPrv&&!$reqDefCvPrdSrv&&$dbRqDfCvPS==="1") {
                $doSavePrv=true;
                $fldarr["reqDefCvPrdSrv"]="0";
            }
        }
    ...
    if (isset($accVerified[0])) {
        $fldarr["verificado"]=$accVerified;
        if (!isset($prvExtraOrder)) $prvExtraOrder=[];
        $prvExtraOrder[]=["name"=>"isnull(edocta)"];
    }
    if (isset($opinionFulfilled[0])) {
        $fldarr["cumplido"]=$opinionFulfilled;
        if (!isset($prvExtraOrder)) $prvExtraOrder=[];
        $prvExtraOrder[]=["name"=>"isnull(opinion)"];
    }
    require_once "clases/Proveedores.php";
    if ($tipoComercial) {
        $fldarr["tipo"]=Proveedores::TIPO_COMERCIAL;
    }
    if ($tipoAduanal) {
        if (!isset($fldarr["tipo"])) $fldarr["tipo"]=0;
        $fldarr["tipo"]|=Proveedores::TIPO_ADUANA;
    }
    if ($tipoTraslado) {
        if (!isset($fldarr["tipo"])) $fldarr["tipo"]=0;
    ...
    echo "\n<!-- FIELD ARRAY: ".json_encode($fldarr)." -->\n";
    global $query;
    $prvObj = new Proveedores();
    ...
    $prvObj->clearOrder();
    if(isset($prvExtraOrder[0])) foreach ($prvExtraOrder as $peIdx => $peItem) {
        $prvObj->addOrder($peItem["name"],$peItem["dir"]??"asc");
    }
...
//                "numpagOpinion"=>$pagecount,
            ...
            "reqObjImp"=>$reqObjImp,
            "reqPayTaxChk"=>$reqPayTaxChk,
            "reqDefCvPrdSrv"=>$reqDefCvPrdSrv,
            "comentarios"=>$textProveedor   //
        ]=$prvData[0];
        if (isset($tiposProveedor)) {
            $tipoComercial=($tiposProveedor&Proveedores::TIPO_COMERCIAL)>0?1:0;
            $tipoAduanal=($tiposProveedor&Proveedores::TIPO_ADUANA)>0?1:0;
            $tipoTraslado=($tiposProveedor&Proveedores::TIPO_FLETE)>0?1:0;
            $tipoLogistica=($tiposProveedor&Proveedores::TIPO_LOGISTICA)>0?1:0;
        }
        ...
        $reqPayTaxChk=($reqPayTaxChk==="1");
        $reqDefCvPrdSrv=($reqDefCvPrdSrv==="1");
        require_once "clases/Usuarios.php";
        $usrObj = new Usuarios();
        $usrData=$usrObj->getData("nombre='$codigoProveedor'",0,"id,email");
