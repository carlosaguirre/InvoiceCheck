		...
		if (is_dir($fileabs)) {
			...
			if (!$cache["enough"]) {
				...
				if ($cfdiObj->has("folio")) {
					...
					if ($folio!==$cfdiFolio) {
						$cfdiSerie=$cfdiObj->get("serie");
						if (!isset($cfdiSerie[0]) || $folio!==($cfdiSerie.$cfdiFolio)) {
							$intFolio=intval($folio);
							$intCfdiFolio=intval($cfdiFolio);
							if ($intFolio<=0 || $intFolio!=$intCfdiFolio) {
								$stats["fileMessages"]++;
								echo "<p>$stats[fileMessages]. $fileabs : <b style='background-color: plum;'>FOLIO INCORRECTO</b> ($cfdiFolio(cfdi) != $folio(filename))";
					...
					if (isset($uuid[4])) {
						...
						if (strtoupper($folio)!==$suffix) {
							$stats["fileMessages"]++;
							echo "<p>$stats[fileMessages]. $fileabs : <b style='background-color: plum;'>UUID INCORRECTO</b> ($cfdiuuid)".getFileInfo($fileabs)."folio(filename): $folio</p>";
...
setlocale(LC_ALL,"America/Mexico_City","America/Mexico_City.utf8","America/Mexico_City.UTF-8","es_MX","es_MX.utf8","es_MX.UTF-8");
