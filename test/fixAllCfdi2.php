...
if (isset($_POST["cmd"])) switch($_POST["cmd"]) {
    case "data": $data=$_POST["data"]??[]; break;
    case "query":
...
<?php
    $idx++;
} else if (isset($data[0])) foreach ($data as $idx=>$row) {
    $rowId=$row["id"];
    $rowSerial=$row["serie"];
    $rowFolio=$row["folio"];
    $rowType=$row["tipoComprobante"];
    $rowSupCod=$row["codigoProveedor"];
    $rowSttIdx=$row["statusn"];
    $rowPath=$row["ubicacion"];
    $rowXML=$row["nombreInterno"];
    $rOldXML=$_POST["oldt"][$idx]["nombreInterno"]??$rowXML;
    $rowPDF=$row["nombreInternoPDF"]??"";
    $rOldPDF=$_POST["oldt"][$idx]["nombreInternoPDF"]??$rowPDF;
    $rowSupReg=$row["rfcEmisor"];
    $rUsers=$row["usuarios"];
?>
              <tr><td><input type="hidden" value="<?=$rowId?>" name="data[<?=$idx?>][id]">
                      <input type="text" value="<?=$rowSerial?>" name="data[<?=$idx?>][serie]" style="width:90px;" readonly></td>
                  ...
                  <td><input type="text" value="<?=$rowSttIdx?>" name="data[<?=$idx?>][statusn]" style="width:80px;" readonly></td>
                  <td><input type="text" value="<?=$rowPath?>" name="data[<?=$idx?>][ubicacion]" readonly></td>
                  <td><input type="text" value="<?=$rowXML?>" name="data[<?=$idx?>][nombreInterno]"<?= ($rowXML===$rOldXML)?"":" class=\"bggreen\"" ?> readonly></td>
                  <td><input type="text" value="<?=$rowPDF?>" name="data[<?=$idx?>][nombreInternoPDF]"<?= ($rowPDF===$rOldPDF)?"":" class=\"bggreen\"" ?> readonly></td>
                  <td><input type="text" value="<?=$rowSupReg?>" name="data[<?=$idx?>][rfcEmisor]" readonly></td>
                  <td><input type="text" value="<?=$rUsers?>" name="data[<?=$idx?>][usuarios]" readonly></td></tr>
<?php } ?>
