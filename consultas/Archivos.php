...
} else if (!validaPerfil("Administrador") && !validaPerfil("Sistemas")) {
    if (getUser()->nombre==="sistemas") doclog("ARCHIVO NO ADMIN","error",["post"=>filterArray($_POST,["target"])]);
...
function shouldReturnJsonAction() {
    return isset($_POST["action"]) || !empty($_POST["hasJson"]);
}
function respondActionError($message, $data=[]) {
    if (shouldReturnJsonAction()) echoJSDoc("error", $message, null, $data, "error");
    else echo "$message<br>";
}

function dispatchGeneralFileActions($accion, $path, $satPath, $isfPath, $ichPath, $mesPath, &$premsg, &$list) {
    switch ($accion) {
        case "descargaZip":
        ...
        case "untar":
            ...
            return 1;
        case "borraArchivo":
            if (getUser()->nombre==="sistemas") doclog("Consultas Archivos borraArchivo","archivo",$_POST);
            $isJson = !empty($_POST['hasJson']);
            $archivo = $_POST["archivoABorrar"]??"";
            if (!isset($archivo[0])) {
                $premsg = "Falta definir archivo a borrar";
                if ($isJson) { echoJSDoc("error", $premsg, null, ["archivo"=>$archivo], "archivo"); return 2; }
            } else if(@unlink($path.$archivo)) {
                $premsg = "Archivo $archivo borrado";
                if ($isJson) { echoJSDoc("success", $premsg, null, ["archivo"=>$archivo], "archivo"); return 2; }
            } else {
                $premsg = "No pudo borrarse el archivo $archivo";
                if ($isJson) { echoJSDoc("error", $premsg, null, ["archivo"=>$archivo], "archivo"); return 2; }
            }
...
function dispatchSupportActions($accion) {
    switch ($accion) {
        case "rompeSello":
            rompeSello();
            return 1;
...
/**
 * Dispatcher de acciones para consultas/Archivos.php
 * Contrato de respuesta recomendado (cuando aplica JSON):
 *   result: success|error|upkeep|refresh|empty
 *   message: string
 *   data: mixed
 *
 * Grupos de acciones:
 * - Descarga/organización: descargaZip, descargaTar, unzip, untar, ordenaSAT, ordenaIS, actualizaSAT, actualizaIS, actualizaArch, actualizaDif, borraArchivo
 * - Documentos: getDoc, addDoc, repDoc, resDoc, delDoc
 * - Pagos/PDF: uuidpdf, massReqPaym, pdfbreak, pdfmerge, pdfmerge2
 * - Otros: rompeSello, recuperaSello, updateTaxStatusProof, appendBackgroundDocs, removeBackgroundPages, extractData
 */
function procesaAccion($accion) {
    $accion = trim((string)$accion);
    if ($accion==="") {
        respondActionError("No se recibió la acción a ejecutar", ["file"=>getShortPath(__FILE__),"function"=>__FUNCTION__,"line"=>__LINE__]);
        return;
    }
    ...
    $handled = dispatchGeneralFileActions($accion, $path, $satPath, $isfPath, $ichPath, $mesPath, $premsg, $list);
    if ($handled===2) return;
    if ($handled===1) {
    ...
    if (dispatchSupportActions($accion)===1) return;
    ...
    switch ($accion) {
        ...
        case "getDoc":
            ...
            $invData=$invObj->getData("id=$invId", 1);
            ...
            return;
        case "addDoc":
            doclog("Archivos:addDoc", "docs", $baseData+["line"=>__LINE__,"files"=>($_FILES??false)]);
            ...
            if ($tipo==="cfdi") {
                ...
                if (!isset($pdf[0])) {
                    $prvRfc=$_POST["rfc"]??"";
                    if (!isset($prvRfc[0])) {
                        $prvData=dao("prv")->getData("codigo='$invData[codigoProveedor]'", 1);
                ...
                dao("fir")->insertRecord(["idUsuario"=>getUser()->id, "modulo"=>"cfdi", "idReferencia"=>$invId, "accion"=>"agrega", "motivo"=>$_POST["motivo"]??$pdf]);
                DBi::commit();
                DBi::autocommit(TRUE);
            ...
            dao("fir")->insertRecord(["idUsuario"=>getUser()->id, "modulo"=>"ea", "idReferencia"=>$invId, "accion"=>"agrega", "motivo"=>$eaPath]);
            DBi::commit();
            DBi::autocommit(TRUE);
        ...
        case "repDoc":
            ...
            $tipo = $_POST["type"]??"";
            unset($baseData["target"]);
            //$_POST_NOTARGET = filter Array($_POST,["target"]);
            ...
            if (!isset($invId[0])) {
                ...
                $ordData=dao("ord")->getData("id=$ordId", 1);
                ...
                $dt = new DateTime();
                if (!rename($filename, $upath.$name.$dt->format("ymdHis").".pdf"))
                    { echoJSDoc("error", "El documento existente no se puede reemplazar", null, $baseData+["line"=>__LINE__,"ubicacion"=>$upath,"moveError"=>error_get_last(),"nombre"=>$name.".pdf"], "error"); return; }
                sleep(3);
                ...
                dao("fir")->insertRecord(["idUsuario"=>getUser()->id, "modulo"=>"orden", "idReferencia"=>$ordId, "accion"=>"reemplaza", "motivo"=>$upath.$name]);
            ...
            if ($tipo==="cfdi") {
                ...
                if (!isset($name[0])) { echoJSDoc("error", "Datos incompletos para generar documento CFDI-PDF", null, $baseData+["line"=>__LINE__,"query"=>$query,"data"=>$invData], "error"); return; }
            } else { // $tipo==="ea"
                $eacp=trim(str_replace("-","",$invData["codigoProveedor"]??""));
                $tmplog.=",eacp='{$eacp}'";
                $eafolio=$invData["folio"]??"";
            ...
            dao("fir")->insertRecord(["idUsuario"=>getUser()->id, "modulo"=>$tipo, "idReferencia"=>$invId, "accion"=>"reemplaza", "motivo"=>$upath.$name]);
        ...
        case "resDoc": // Recupera archivo borrado
            ...
            if (!$esDesarrollo) { echoJSDoc("error", "No tiene permiso para restaurar documentos eliminados", null, $baseData+["line"=>__LINE__,"user"=>getUser()->nombre], "error"); return; }
            $invId=$_POST["id"];
        ...
        case "delDoc":
            ...
            $invId=$_POST["id"];
            if (!$esBorraDoc) {
                // todo: checar si es pendiente si puede eliminarlo
                echoJSDoc("error", "No tiene permiso para eliminar documentos", null, $baseData+["line"=>__LINE__,"user"=>getUser()->nombre], "error");
                return;
            }
            $tipo=$_POST["type"];
            if(!isset($tipo[0])) { echoJSDoc("error", "No se especificó tipo de documento", null, $baseData+["line"=>__LINE__], "error"); return; }
            ...
            if ($tipo==="cfdi") {
                ...
                if ($invObj->saveRecord(["id"=>$invId,"nombreInternoPDF"=>null])) {
                    ...
                    dao("fir")->insertRecord(["idUsuario"=>getUser()->id, "modulo"=>"cfdi", "idReferencia"=>$invId, "accion"=>"elimina", "motivo"=>$_POST["motivo"]??""]);
            ...
            if ($invObj->saveRecord(["id"=>$invId,"ea"=>"0"])) {
                ...
                if (getUser()->name==="admin") {
                    $usrData = dao("usr")->getData("nombre='SISTEMAS'", 1, "id");
                    if (isset($usrData[0]["id"][0])) $usrId=$usrData[0]["id"];
                }
                dao("fir")->insertRecord(["idUsuario"=>$usrId, "modulo"=>"ea", "idReferencia"=>$invId, "accion"=>"elimina", "motivo"=>$_POST["motivo"]??""]);
            ...
            break;
        case "massReqPaym": //doMassRequirePayment();
            ...
            if (isset($files[0])) {
                try {
                    ...
                    foreach ($onePages as $oneIdx => $pdfname) { // Generar archivos txt por cada pdf generado
                        ...
                        $txtPages[$oneIdx]=$basn.".txt";
                        $cmd="\"C:\\Program Files\\XPdfTools\\bin64\\pdftotext.exe\" \"$pdfname\"";
                        $out=exec($cmd);
                    }
                } catch (Exception $ex) {
                    echoJSDoc("error", "Ocurrió un error", null, $baseData+["line"=>__LINE__,"error"=>getErrorData($ex),"logs"=>$log], "error");
                }
                sleep(1);
            }
                ...
                try {
                    if (file_exists($textname)) {
                        ...
                        if ($handle) {
                                ...
                                if ($isRefLine===false) {
                                    ...
                                    if ($isRefLine>0) {
                                        ...
                                        if ($pos!==false && $isRefLine>1) {
                                            ...
                                            if ($pos2!==false && $pos2>$pos) $isRefLine=0;
                                        }
                                    }
                                    if ($isRefLine===0 && substr($line, 0, 3)==="SOL") {
                                        $txt=substr($line, 3);
                                        $fecha = substr($txt, 0, 4);
                                        $gpoCut = substr($txt, 6, 3);
                                        $preFolio=$gpoCut.$fecha;
                                        ...
                                        if (file_exists($localPath.$newName)) {
                                            ; // break 2, o renombrar para tener varias versiones, pero se podría saturar el espacio con muchas copias
                                            // o eliminar para sustituir con el nuevo archivo
                                            unlink($localPath.$newName);
                                        }
                                        if (rename($localPath.$onepages[$tIdx],$localPath.$newName)) {
                            ...
                            doclog("No se pudo leer archivo de texto","masspaym",["onepageIdx"=>$tIdx,"pdfname"=>$onepage[$tIdx],"textname"=>$textname]);
                            continue;
                        }
                    } else {
                        doclog("No existe archivo de texto","masspaym",["onepageIdx"=>$tIdx,"pdfname"=>$onepage[$tIdx],"textname"=>$textname]);
                        continue;
                    }
                } catch (Exception $ex) {
                    doclog("Error al obtener comprobante de pago masivo","masspaym",["onepageIdx"=>$tIdx,"pdfname"=>$onepage[$tIdx],"textname"=>$textname,"error"=>getErrorData($ex)]);
                    continue;
                }
            }
            $foundFiles=0;
            ...
            foreach ($data as &$value) {
                ...
                if (file_exists($localPath.$value["name"])) {
                    //
                    try {
                        $newName=payRequest(...array_values($value));
                        DBi::commit();
                        $value["comprobantePago"]=$newName;
                        $foundFiles++;
                    } catch (Error $err) {
                        DBi::rollback();
                        doclog("Error al marcar pagada una solicitud", "error", ["error"=>getErrorData($err),"value"=>$value]);
                    }
                }
            }
            DBi::autocommit(true);
            $returnData=["folios"=>array_column($data, "folio"),"data"=>$data,"onePages"=>$onePages,"txtPages"=>$txtPages,"foundPages"=>$foundPages];
            if ($foundFiles==0) { echoJSDoc("error", "No se encontró ningún comprobante de pago relacionado", null, $returnData, "error"); break; } // return; }
            $dataLen=count($data);
            if ($foundFiles===$dataLen) { echoJSDoc("success", "Comprobantes encontrados ya generados se ligaron a las solicitudes marcadas", null, $returnData, "archivo"); break; } // return; }
        ...
        case "pdfbreak":
            ...
            if ($onePageFiles===null) break; // return;
            require_once "clases/PDFTools.php";
            $localPath=PDFTools::getSavePath();
            $webPath=PDFTools::getWebPath();
            ...
            if ($textFiles===null) break; // return;
            sleep(1);
        ...
        case "pdfmerge":
            try {
                ...
                if (empty($mergeList)) { echoJSDoc("error", "Falta indicar una lista de archivos",null, $baseData+["line"=>__LINE__], "error"); break; }
                require_once "clases/PDFTools.php";
                PDFTools::init();
                $resultData=[];
                $pt=PDFTools::create();
            ...
            break; // return;
        default:
            respondActionError("Acción no reconocida", $baseData+["line"=>__LINE__]);
            return;
    }
    if(isset($premsg)) echo $premsg."<br>";
    if(isset($list)) echo arr2List($list, "OL");
...
function getSolFolioInLine($txt) {
    return getReqPymFolio($txt, false, true);
}
function payRequest($id,$folio,$type,$oriname) {
    global $query;
    ...
    $solObj=dao("sol");
    $solData=$solObj->getData("id=$id", 1, "idFactura,idOrden,status,proceso");
    if (!isset($solData[0])) throw new DocLogException("No se encontró la solicitud $folio", ["id"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"query"=>$query]);
    $solData=$solData[0];
    switch($type[0]) {
        case "f": $invId=$solData["idFactura"];
            $invData=dao("inv")->getData("id=$invId", 1, "ubicacion,nombreInterno,nombreInternoPDF");
            if (!isset($invData[0])) throw new DocLogException("No se encontró la factura relacionada", ["solId"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"solData"=>$solData,"query"=>$query]);
            $invData=$invData[0];
            $path=$invData["ubicacion"];
            $name="CP_".(isset($invData["nombreInternoPDF"][0])?$invData["nombreInternoPDF"]:$invData["nombreInterno"]).".pdf";
            if (!dao("inv")->saveRecord(["id"=>$invId,"comprobantePagoPDF"=>$name]))
                throw new DocLogException("No se pudo asignar el comprobante de pago a la factura", ["solId"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"solData"=>$solData,"invData"=>$invData,"query"=>$query]);
        break;
        case "o": $ordId=$solData["idOrden"];
            $ordData=dao("ord")->getData("id=$ordId", 1, "rutaArchivo,nombreArchivo");
            if (!isset($ordData[0])) throw new DocLogException("No se encontró la orden relacionada", ["solId"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"solData"=>$solData,"query"=>$query]);
            $ordData=$ordData[0];
            $path=$ordData["rutaArchivo"];
            $name="CP_".$ordData["nombreArchivo"];
            if (!dao("ord")->saveRecord(["id"=>$ordId,"comprobantePago"=>$name]))
                throw new DocLogException("No se pudo asignar el comprobante de pago a la orden", ["solId"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"solData"=>$solData,"ordData"=>$ordData,"query"=>$query]);
        break;
        case "c": $ctrId=$solData["idContrarrecibo"];
            $ctrData=dao("ctr")->getData("id=$ctrId", 1, "aliasGrupo,fechaRevision,folio");
            if (!isset($ctrData[0])) throw new DocLogException("No se encontró el contra recibo relacionado", ["solId"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"solData"=>$solData,"query"=>$query]);
            $ctrData=$ctrData[0];
            ...
            if (!dao("ctr")->saveRecord(["id"=>$ctrId,"comprobantePago"=>$name]))
                throw new DocLogException("No se pudo asignar el comprobante de pago al contra recibo", ["solId"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"solData"=>$solData,"ctrData"=>$ctrData,"query"=>$query]);
        break;
        default:
            throw new DocLogException("Tipo de documento invalido", ["solId"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"solData"=>$solData]);
    }
    $docRoot=$_SERVER["DOCUMENT_ROOT"];
    $fullPath=$docRoot.$path;
    $fullNewName=$fullPath.$name;
    if (file_exists($fullNewName)) {
        ...
        if (!chmod($fullNewName, 0777)) throw new DocLogException("No se pudo asignar nuevo comprobante, ya existe uno",["solId"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"newname"=>$path.$name]);
        if (!unlink($fullNewName)) throw new DocLogException("No se pudo asignar nuevo comprobante, ya existe uno",["solId"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"newname"=>$path.$name]);
    }
    if (!rename($fullOriName, $fullNewName)) throw new DocLogException("No se pudo guardar el comprobante de pago",["solId"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"newname"=>$path.$name]);
    if (!$solObj->saveRecord(["id"=>$solId,"proceso"=>SolicitudPago::PROCESO_PAGADA])) throw new DocLogException("No se pudieron guardar los cambios a la solicitud",["solId"=>$id,"folio"=>$folio,"type"=>$type,"oriname"=>$oriname,"solData"=>$solData,"query"=>$query]);
...
function readingTextFiles($txtn) {
    $data=[]; $log=[]; $webPath="archivos/"; $webnames=[];
    foreach ($txtn as $tIdx => $textname) {
        $block="";
        try {
            if (file_exists($textname)) {
                $handle=fopen($textname, "r");
                ...
                $log[]="Text File wasn't created '$textname'";
                continue;
            }
        } catch (Exception $ex) {
...
function doMassRequirePayment() {
    $tmpPath = dirname($_SERVER['DOCUMENT_ROOT'])."/docs/tmp/";
    $webPath="$_SERVER[HTTP_ORIGIN]/docs/tmp/";
    $invPath = $_SERVER['DOCUMENT_ROOT'];
    $tmpLen=strlen($tmpPath);
    $refList=[]; $mapSol=[];
    $data=$_POST["data"];
    foreach ($data as $dataIndex => $dataItem) {
        $dataFolio=$dataItem["folio"];
        $refList[]=$dataFolio;
        $mapSol[$dataFolio]=$dataItem; // id, folio, type // procesoId, statusId, grupoId, provId, authList, monto, mon
    }
    // Obtener archivo con recibos bancarios
    $files=getFixedFileArray($_FILES["files"]); // obtener archivos con recibos bancarios, formato específico: Informe de detalle de pagos de iniciación de transacciones
    $localFiles=[]; $textFiles=[]; $baseFiles=[]; // $log=[]; $webFiles=[];
    try {
        require_once "clases/PDFTools.php";
        $pt=PDFTools::create(); 
    } catch (Exception $ex) {
        echoJSDoc("error", "Error al inicializar extraccion de comprobantes", null, $baseData+["line"=>__LINE__,"error"=>getErrorData($ex)], "error");
        return;
    }
    $log=[];
    foreach ($files as $fIdx => $file) {
        if ($fIdx>0) sleep(1); // para no empalmar los nombres de varios archivos
        $resultData[]=$pt->break($file["tmp_name"]); // savePath=/docs/tmp/
        $baseFiles[]=$resultData["basename"];
        array_push($localFiles, ...$resultData["local"]);
    }
    $hasTextFileCreated=false; $createTextFileErrors=[];
    foreach ($localFiles as $localIndex => $localFile) {
        try {
            $textFiles[$localIndex]=substr($localFile, 0, -4).".txt";
            $out=exec("\"C:\\Program Files\\XPdfTools\\bin64\\pdftotext.exe\" \"$localFile\"");
            $hasTextFileCreated=true;
        } catch (Exception $ex) {
            $createTextFileErrors[]=getErrorData($ex);
        }
    }
    if ($hasTextFileCreated) {
        if (isset($createTextFileErrors[0])) doclog("Errores al convertir PDF a Texto","error",$baseData+["line"=>__LINE__,"errors"=>$createTextFileErrors]);
        sleep(1);
    } else if (isset($createTextFileErrors[0])) { echoJSDoc("error", "Errores al convertir PDF a Texto", null, $baseData+["line"=>__LINE__,"errors"=>$createTextFileErrors], "error"); return; }
    $result=[]; $storeReceiptErrors=[];
    global $query;
    $solObj=dao("sol");
    foreach ($baseFiles as $baseIndex => $baseFile) {
        $rpMap=getReqPymMap($baseFile, $refList); // [<solFolio>=><docsTempComprobantePago>, ..., error=>[message, idx, filename]|[message,code,file,line,trace]]
        foreach($rpMap as $reqName => $paymentReceiptName) {
            if ($reqName=="error") { // validar si aqui se mandan los errores por doclog(...,error,...) o en otro punto
                continue; // ToDo: hacer lista para mostrar errores
            }
            if (!isset($mapSol[$reqName])) {
                // mandar error, q no existe el folio en el mapa de solicitudes
                continue;
            }
            $reqData=$mapSol[$reqName]; // id, type(f,o,c)
            $solData=$solObj->getData("id=$reqData[id]", 1);
            if (isset($solData[0])) $solData=$solData[0];
            else {
                // mandar error que no se encontró informacion de la solicitud
                continue;
            }
            switch($reqData["type"][0]) {
                case "f":
                    $idFactura=$solData["idFactura"];
                    $tabObj=dao("inv");
                    $invData=$tabObj->getData("id=$idFactura", 1, "ubicacion,nombreInternoPDF,comprobantePagoPDF");
                    if (isset($invData[0])) $invData=$invData[0];
                    else { /* ERROR */ }
                    // toDo: decidir q hacer si ya existe el archivo, si se le agrega un timestamp o si solo se sobreescribe
                    $path=$invData["ubicacion"];
                    $cpName="CP_$invData[nombreInternoPDF]";
                    if (rename($tmpPath.$paymentReceiptName, $invPath.$path.$cpName.".pdf")===false) {
                        $storeReceiptErrors[]=["message"=>"No se pudo guardar el archivo","docTmpName"=>$paymentReceiptName,"ubicacion"=>$path,"cpName"=>$cpName,"invId"=>$idFactura];
                        break;
                    }
                    if ($tabObj->saveRecord(["id"=>$idFactura, "comprobantePagoPDF"=>$cpname,"statusn"=>new DBExpression("statusn|".Facturas::STATUS_PAGADO),"status"=>"Pagado"])) {
                        dao("prc")->cambioFactura($idFactura, "Pagado", getUser()->nombre, false, "Solicitud $reqName con Factura");
                    }
                    break;
                case "o":
                    $idOrden=$solData["idOrden"];
                    $tabObj=dao("ord");
                    $ordData=$tabObj->getData("id=$idOrden", 1, "rutaArchivo, nombreArchivo, comprobantePago");
                    if (isset($ordData[0])) $ordData=$ordData[0];
                    else { /* ERROR */ }
                    $path=$ordData["rutaArchivo"];
                    $cpName="CP_$ordData[nombreArchivo]";
                    $pdfField="nombreArchivo"; $fileField="comprobantePago"; $pathField="rutaArchivo";
                    $divName="ordDocs"; $fieldId="idOrden"; $addFldArr=["status"=>new DBExpression("status|".OrdenesCompra::STATUS_PAGADO)];
                    break;
                case "c":
                    $tabObj=dao("ctr"); $fileField="comprobantePago"; $divName="ctrDocs"; $fieldId="idContrarrecibo";
                    break;
            }
        }
    }
    ...
    foreach ($textFiles as $tIdx => $textName) {
       if (file_exists($textName)) {
            $handle=fopen($textName, "r");
            if ($handle) {
                $isRefLine=false; // $isSomething=false;
                // Localizar la línea de la referencia de solicitud relacionada
                while (($line=fgets($handle))!==false) {
                    $line=trim($line);
                    if (!isset($line[0])) continue;
                    if (strpos($line, "mero de Referencia de Transacci")!==false) $isRefLine=true; // Número de Referencia de Transacción
                    // else if (strpos($line, "")!==false) $isSomething=true;
                    else if ($isRefLine) {
                        foreach ($refList as $refIdx => $refVal) {
                            // Corroborar la referencia con un folio de solicitud
                            if (strpos($line, $refVal)!==false) {
                                switch($solType[0]) { } global $query;
                                $solData=$solObj->getData("id=$solId", 1);
                                if (isset($solData[0])) $solData=$solData[0];
                                if (isset($pathField[0])&&isset($pdfField[0])) {
                                    $tabData=$tabObj->getData("id=".$solData[$fieldId], 1, "$pathField,$pdfField,$fileField");
                                    if (isset($tabData[0])) {
                                        $path=$tabData[0][$pathField];
                                        $name=$tabData[0][$pdfField];
                                        $cpName=$tabData[0][$fileField]??"";
                                        if (!isset($cpName[0])) $cpName="CP_".$name;
                                        global $query;
                                        $fieldarray=["id"=>$solData[$fieldId], $fileField=>$cpName];
                                        if (isset($addFldArr))
                                            $fieldarray+=$addFldArr;
                                        if ($tabObj->saveRecord($fieldarray)||empty(DBi::$errno)) {
                                            $cpName.=".pdf";
                                            $docroot=$_SERVER["DOCUMENT_ROOT"];
                                            $http=$_SERVER["HTTP_ORIGIN"];
                                            $basePath=($path??"");
                                            $absPath=$docroot.$basePath;
                                            if (file_exists($absPath.$cpName)) {
                                                chmod($absPath.$cpName, 0777);
                                                unlink($absPath.$cpName);
                                            }
                                            if (copy($resultData["local"][$txtIdx],$absPath.$cpName)) {
                                                if ($solType[0]=="c") {
                                                    $ctrData=dao("ctr")->getData("id=".$solData[$fieldId],0,"aliasGrupo,folio");
                                                    if (isset($ctrData[0])) $ctrData=$ctrData[0];
                                                    $ctfData=dao("ctf", ["rows_per_page"=>0])->getData("idContrarrecibo=".$solData[$fieldId]." and autorizadaPor is not null",0,"idFactura");
                                                    if (isset($ctfData[0])) {
                                                        $cfi=array_column($ctfData, "idFactura");
                                                        if (dao("inv")->saveRecord(["id"=>$cfi,"status"=>"Pagado","statusn"=>new DBExpression("statusn|".Facturas::STATUS_PAGADO)])&&isset($ctrData["folio"])) {
                                                            dao("prc")->cambioFactura($cfi, "Pagado", getUser()->nombre, false, "Sol $solFolio x Contra $ctrData[aliasGrupo]-$ctrData[folio] Marcado");
                                                        }
                                                    }
                                                }
                                                if ($solObj->saveRecord(["id"=>$solId,"status"=>new DBExpression("status|".SolicitudPago::STATUS_PAGADA),"proceso"=>SolicitudPago::PROCESO_PAGADA]) || empty(DBi::$errno)) {
                                                    $usrObj=dao("usr");
                                                    $usrData=$usrObj->getData("id=$solData[idUsuario]",0,"nombre,persona,email");
                                                    ...
                                                    if (isset($codigoProveedor[0])) {
                                                        ...
                                                        if (isset($uprData[0])) $uprData=$uprData[0];
                                                    }
                                                    $solObj->firma($solId,"paga");
                                                    DBi::commit();
                                                    DBi::autocommit(true);
                        ...
                        $isRefLine=false;
                        break; // Nada más que buscar. Cuando se agregue la validación del total hay que quitar esta línea
                    } // else if ($isSomething) {}
                } // while line
            } // if handle
        } // if file_exists textName
    } // foreach $textFiles
    echoJSDoc("success", "exito", null, ["webnames"=>$webnames,"logs"=>$log,"text"=>$data]);
}

function replaceDoc() {
    $baseData=["file"=>getShortPath(__FILE__),"function"=>__FUNCTION__]+$_POST;
    $esSistemas = validaPerfil(["Sistemas","Administrador"]);
    if (!$esSistemas) { echoJSDoc("error", "No tiene permiso para reemplazar documentos", null, $baseData+["line"=>__LINE__,"user"=>getUser()->nombre], "error"); return; }
    if (!isset($_POST["path"][0])) { echoJSDoc("error", "Debe incluir la ruta del archivo a reemplazar", null, $baseData+["line"=>__LINE__]); return; }
    $path=$_POST["path"];
    if (!isset($_POST["name"][0])) { echoJSDoc("error", "Debe incluir el nombre del archivo a reemplazar", null, $baseData+["line"=>__LINE__]); return; }
    $name=$_POST["name"];
    if (!isset($_FILES["file"])) { echoJSDoc("error", "Debe agregar un archivo para reemplazar el antiguo", null, $baseData+["line"=>__LINE__,"files"=>$_FILES]); return; }
    $ffile=$_FILES["file"];
    if ($ffile["error"]>0) {
        $fileError=fileCodeToMessage($ffile["error"],["filename"=>$ffile["name"]]);
        if (isset($fileError)) echoJSDoc("error", $fileError, null, $baseData+["line"=>__LINE__,"files"=>$_FILES], "error");
        else echoJSDoc("error", "Error de archivo $ffile[error]", null, $baseData+["line"=>__LINE__,"files"=>$_FILES], "error");
        return;
    }
    $sysPath=$_SERVER['DOCUMENT_ROOT'];
    ...
    if (file_exists($filename)) {
        $dt = new DateTime();
        if (!rename($filename, $sysPath.$path.$name.$dt->format("ymdHis").".pdf")) {
            echoJSDoc("error", "El archivo ya existe y no se puede reemplazar", null, $baseData+["line"=>__LINE__,"filename"=>$filename,"moveError"=>error_get_last()], "error");
            return;
        }
        sleep(4);
    }
    if (move_uploaded_file($ffile["tmp_name"], $filename)===false)
        echoJSDoc("error", "El archivo no se pudo cargar", null, $baseData+["line"=>__LINE__,"filedata"=>$ffile,"filename"=>$filename,"moveError"=>error_get_last()], "error");
    else
        echoJSDoc("success", "Archivo reemplazado satisfactoriamente");
}
...
function extrae($dwnPath, $ext) {
    ...
    foreach($xtfiles as $filename) {
        ...
        try {
            ...
            print_r($filename,true).": ".$e->getMessage();
        }
...
function recuperaSello() {
    ...
    if (isset($solId[0])) {
        ...
        if (isset($solData[0])) {
            ...
            if ($solStatus>=SolicitudPago::STATUS_CANCELADA) {
                ...
                // toDo: Falta recuperar sello por contra recibo
                // $resQuery="UPDATE contrarrecibos SET selloImpreso=0 WHERE id=$ctrId"; // in (22323, 22324)
            }
        }
    }
    if (empty($resQuery)) {
    ...
    if (!empty($resQuery)) {
        if ($result==="success") {
            dao("prc")->cambiaSolicitud($solId,"restauraSello","");
            DBi::commit();
        } else DBi::rollback();
        DBi::autocommit(true);
    }
...
function rompeSello() {
    ...
    if (empty($tieneSello)) {
        ...
        if (is_bool($rv)) { // 
            ...
            dao("prc")->cambiaSolicitud($solId,"rompeSello","evento en 5 minutos");
...
function updateTaxStatusProof() {
    ...
    $alias=$_POST["alias"];
    $incSep=$_POST["inclusiveSeparator"]??"***";
    $docTimes=0;
    $aliasIdx=strpos($alias, "_");
    if ($aliasIdx>0) {
        $docTimes=+substr($alias, $aliasIdx+1);
        $alias=substr($alias, 0, $aliasIdx);
    }
    if ($alias==="SAC") $alias="SERVICIOS";
    $gpoId=$_POST["gpoId"];
    $ffile=$_FILES["file"];
    $docRoot = $_SERVER["DOCUMENT_ROOT"];
    if (is_array($ffile["name"])) $ffile=getFixedFileArray($ffile)[0];
    
    $pdfname=$ffile["name"];
    $txtname=substr($pdfname, 0, -4).".txt";
    $tmpfile=$docRoot."/docs/tmp/".$pdfname;
    $txtfile=$docRoot."/docs/tmp/".$txtname;
    $docPath=$docRoot."/docs/csf/";
    if (file_exists($tmpfile)) {
        //$dt = new DateTime();
        //echoJSDoc("upkeep","Temporal file exists... renaming it...",$incSep,["tmpfile"=>$tmpfile,"date"=>$dt->format("ymdHis")],"archivo");
        //rename($tmpfile, $docRoot."/docs/tmp/".substr($pdfname, 0, -4).$dt->format("ymdHis").".pdf");
        //sleep(3);
        if (unlink($tmpfile)) {
            echoJSDoc("upkeep","Old PDF file deleted",$incSep,$baseData+["line"=>__LINE__,"filename"=>$tmpfile],"archivo");
        } else
            echoJSDoc("upkeep","Old PDF file can't be deleted",$incSep,$baseData+["line"=>__LINE__,"filename"=>$tmpfile],"archivo");
    } else {
        echoJSDoc("upkeep","Loaded File doesn't exist",$incSep,$baseData+["line"=>__LINE__,"pdfname"=>$pdfname,"txtname"=>$txtname]);
    }
    if (file_exists($txtfile)) {
        if (unlink($txtfile))
            echoJSDoc("upkeep","Old Text file deleted",$incSep,["filename"=>$txtfile],"archivo");
        else
            echoJSDoc("upkeep","Old Text file can't be deleted",$incSep,["filename"=>$tmpfile],"archivo");
    } else {
        echoJSDoc("upkeep","Text File doesn't exist",$incSep,$baseData+["line"=>__LINE__,"txtfile"=>$txtfile]);
    }
    sleep(3);
    echoJSDoc("upkeep", "INITIAL DATA READY", $incSep, $baseData+["line"=>__LINE__,"lastPath"=>$lastPath,"alias"=>$alias,"docTimes"=>$docTimes,"gpoId"=>$gpoId,"pdfname"=>$tmpfile,"txtname"=>$txtfile], "archivo");
    exec("ForFiles /p \"C:\\Apache24\\htdocs\\docs\\tmp\" /s /d -3 /c \"cmd /c del /q @file\"");
                ...
                while (($line=fgets($handle))!==false) {
                    ...
                    if ($found) {
                        ...
                        if ($yrpos===false) {
                            ...
                            echoJSDoc("upkeep","Last year check",$incSep,$baseData+["line"=>__LINE__,"txtline"=>$line,"datetext"=>$datetext,"yrpos"=>$yrpos,"yr"=>$yr,"lastyr"=>$lastyr],"archivo");
                        ...
                        if ($datepath===false) {
                            echoJSDoc("upkeep","Date Path Failed",$incSep,$baseData+["line"=>__LINE__,"txtline"=>$line,"datetext"=>$datetext,"yrpos"=>$yrpos,"monpos"=>$monpos,"monthname"=>$monthname],"archivo");
                            return false;
                        }
...
// ToDo: si found es false hacer return false pero result=askYear. En javascript mostrar dialogo donde se pida el mes y año y hacer nuevo submit para copiar archivo a la ruta especificada con el mes y el año. Si found es true se calcula mes y año para copiar arcihvo a la ruta calculada.
                ...
                if ($found && isset($datepath[0])) {
                    if ($docTimes>0) $aliasN="{$alias}_{$docTimes}";
                    else $aliasN=$alias;
                    echoJSDoc("upkeep","File Parsed",$incSep,$baseData+["line"=>__LINE__,"datefound"=>$datetext,"dir"=>$datepath,"alias"=>$aliasN],"archivo");
                    ...
                    dao("gpo")->saveRecord(["id"=>$gpoId,"conSitFis"=>$datepath,"conSitFisTimes"=>$docTimes]);
                    dao("prc")->alta("Grupo",$gpoId,"Actualiza",$datepath.$aliasN);
    ...
    if (!isset($text)) { echoJSDoc("error", "No se encontró la fecha", null, null, "error); return false; }
    ...
    if ($type==="Todas") {
        ...
        if (isset($solData["archivoAntecedentes"][0])) {
            ...
            if ($iniPage==1 && $endPage==$pdfObj->pageCount) {
                if (!$solObj->saveRecord(["id"=>$solId,"archivoAntecedentes"=>null])) { echoJSDoc("error", "No se pudo guardar la solicitud $solId", null, $baseData+["line"=>__LINE__,"query"=>$query], "error"); return; }
                dao("prc")->alta("SolPago",$solId,"Antecedentes","Elimina {$delPages}/{$pageCount} hojas");
...
function addBGDocs() {
    ...
    if (!move_uploaded_file($ffile["tmp_name"], $destination)) { echoJSDoc("error", "El archivo PDF no se pudo descargar",null, $baseData+["line"=>__LINE__,"moveError"=>error_get_last(),"filedata"=>$ffile], "error"); }
    ...
    if (!dao("sol")->saveRecord(["id"=>$solId,"archivoAntecedentes"=>$bgName])) { echoJSDoc("error", "No se pudo guardar el archivo", null, $baseData+["line"=>__LINE__,"tmpname"=>$destination,"query"=>$query,"filedata"=>$ffile], "error"); return; }
    $newPageCount=$oldPageCount+$addingPages;
    $pl_s=$addingPages==1?"":"s";
    dao("prc")->alta("SolPago",$solId,"Antecedentes",$oldPageCount==0?"Nuevo $addingPages hoja{$pl_s}":"Agrega {$addingPages} hoja{$pl_s}, ahora son {$newPageCount}");
