...
function loadCSV(name) {
  ...
  let tgtid="loadCSV";
  if (name==="clientes") tgtid+="Clientes";
  else if (name==="comparaavance") tgtid+="Avance";
  else if (name==="comparasat") tgtid+="SAT";
  else return;
  let tgt=ebyid(tgtid);
  let file=tgt.files[0];
  let url="consultas/comparaSAT.php";
  let parameters={accion:"alta",name:name,file:file};
    ...
    if (state===4) {
      if (status==200) {
        try {
          ...
          if (jobj.result) {
            if (jobj.result==="error" && jobj.message) {
              overlayMessage("<p>Ocurrió un error: "+jobj.message+"</p>","ERROR");
            } else if (jobj.inserted || jobj.inserted===0) {
  ...
  let errorFunc=function(reasonText, parameters, evt) {
    console.log("ERR: "+reasonText+", PARAMS:",parameters,", EVENT:",evt);
...
function consultaAvance() {
    console.log("INI function consultaAvance");
    let url="consultas/comparaSAT.php";
    let parameters={accion:"consultaAvance"};
    let successFunc=function(resultText, params, state, status) {
      if(state==4&&status==200){
        let msgObj={eName:"DIV"};
        overlayMessage("RESULT: <pre>"+resultText+"</pre>","SUCCESS");
      }
    };
    let errorFunc=function(reasonText, parameters, evt) {
      console.log("ERR: "+reasonText+", PARAMS:",parameters,", EVENT:",evt);
      overlayMessage("REASON: <pre>"+reasonText+"</pre>","ERROR");
    };
    postService(url,parameters,successFunc,errorFunc);
}
function consultaSAT() {
    console.log("INI function consultaSAT");
    let url="consultas/comparaSAT.php";
    let parameters={accion:"consultaSAT"};
    let successFunc=function(resultText, params, state, status) {
      if(state==4&&status==200){
        overlayMessage("RESULT: <pre>"+resultText+"</pre>","SUCCESS");
      }
...
function muestraDatos(tabla,recargar) {
  ...
  if (!tBody||recargar) {
    let parameters={tableid:tabla.id,tablename:tabla.getAttribute("name"),accion:"consulta"};
    if (!tBody) {
  ...
  if (state>=4) {
    ...
    if (resultText.length>0) {
      ...
      if (jobj) {
        ...
        if (ultElem && curult!==newult) {
          ekfil(ultElem);
          ultElem.appendChild(ecrea({eText:newult.toString()}));
        }
        if (pagElem && curpag!==newpag) {
          ekfil(pagElem);
...
function comparaDatos() {
  ...
  let parameters={accion:"comparaAmbas",finicio:aIni+"-"+mIni+"-"+dIni+" 00:00:00",ffin:aFin+"-"+mFin+"-"+dFin+" 23:59:59"};
      ...
      if (status==200) {
        try {
          ...
          if (jobj.result) { // && jobj.result==="success"
            let msgarr=[ecrea({eName:"H2",className:"centered marginH5",eText:"Comparativo Avance-SAT, del "+fechaInicio+" al "+fechaFin}),ecrea({eName:"TABLE",className:"maxfit centered screen",eChilds:[{eName:"THEAD",eChilds:[{eName:"TR",eChilds:[{eName:"TH",eText:"Sólo Avance"},{eName:"TH",eText:"Sólo SAT"},{eName:"TH",eText:"Diferentes"},{eName:"TH",eText:"Iguales"},{eName:"TH",eText:"Total Avance"},{eName:"TH",eText:"Total SAT"}]}]},{eName:"TBODY",eChilds:[{eName:"TR",eChilds:[{eName:"TD",className:"maxfit",eText:jobj.numSoloAvance.toString()},{eName:"TD",eText:jobj.numSoloSat.toString()},{eName:"TD",eText:jobj.numDiferentes.toString()},{eName:"TD",eText:(+jobj.numAmbos1+jobj.numAmbos2).toString()},{eName:"TD",eText:jobj.totavn.toString()},{eName:"TD",eText:jobj.totsat.toString()}]}]}]})];
            overlayMessage(msgarr,"Resultado entre "+fechaInicio+" y "+fechaFin);
            let csvContent="Avance,SAT,UUID,FECHA,REMISION,RECEPTOR,TIPO,SUBTOTAL,IMPUESTOS,TOTAL\n";
            ...
            if ((+jobj.numSoloAvance)>0) {
                ...
                for(let i=0; i<jobj.soloAvance.length; i++) {
                    ...
                    onlyAvnBody.eChilds.push({eName:"TR",className:"csvRow",eChilds:[{eName:"TD",className:"hidden",eText:"SI"},{eName:"TD",className:"hidden",eText:"NO"},{eName:"TD",className:"maxfit",eText:row.uuid},{eName:"TD",eText:row.fecha},{eName:"TD",eText:row.remision},{eName:"TD",eText:row.receptor},{eName:"TD",eText:row.tipo},{eName:"TD",eText:row.subtotal},{eName:"TD",eText:row.impuestos},{eName:"TD",eText:row.total}]});
                    csvContent+="SI,NO,"+row.uuid+","+row.fecha+","+row.remision+","+row.receptor+","+row.tipo+","+row.subtotal+","+row.impuestos+","+row.total+"\n";
                }
                appendMessageElement(dra, [ecrea({eName:"H2",className:"centered marginHSp",eText:"Sólo Avance: "+jobj.numSoloAvance+"/"+jobj.soloAvance.length}), ecrea({eName:"TABLE", className:"maxfit centered screen",eChilds:[{eName:"THEAD",eChilds:[{eName:"TR",className:"csvRow",eChilds:[{eName:"TH",className:"hidden",eText:"Avance"},{eName:"TH",className:"hidden",eText:"SAT"},{eName:"TH",eText:"UUID"}, {eName:"TH",eText:"FECHA"}, {eName:"TH",eText:"REMISION"}, {eName:"TH",eText:"RECEPTOR"}, {eName:"TH",eText:"TIPO"}, {eName:"TH",eText:"SUBTOTAL"}, {eName:"TH",eText:"IMPUESTOS"}, {eName:"TH",eText:"TOTAL"}]}]},onlyAvnBody]})]);
            }
            if ((+jobj.numSoloSat)>0) {
                let onlySATBody={eName:"TBODY",eChilds:[]};
                for(let i=0; i<jobj.soloSat.length; i++) {
                    let row=jobj.soloSat[i];
                    onlySATBody.eChilds.push({eName:"TR",className:"csvRow",eChilds:[{eName:"TD",className:"hidden",eText:"NO"},{eName:"TD",className:"hidden",eText:"SI"},{eName:"TD",className:"maxfit",eText:row.uuid},{eName:"TD",eText:row.fecha},{eName:"TD",eText:row.remision},{eName:"TD",eText:row.receptor},{eName:"TD",eText:row.tipo},{eName:"TD",eText:row.subtotal},{eName:"TD",eText:row.impuestos},{eName:"TD",eText:row.total}]});
                    csvContent+="NO,SI,"+row.uuid+","+row.fecha+","+row.remision+","+row.receptor+","+row.tipo+","+row.subtotal+","+row.impuestos+","+row.total+"\n";
                }
                appendMessageElement(dra, [ecrea({eName:"H2",className:"centered marginHSp",eText:"Sólo SAT: "+jobj.numSoloSat+"/"+jobj.soloSat.length}), ecrea({eName:"TABLE",className:"maxfit centered screen",eChilds:[{eName:"THEAD",eChilds:[{eName:"TR",eChilds:[{eName:"TH",className:"hidden",eText:"Avance"},{eName:"TH",className:"hidden",eText:"SAT"},{eName:"TH",eText:"UUID"}, {eName:"TH",eText:"FECHA"}, {eName:"TH",eText:"REMISION"}, {eName:"TH",eText:"RECEPTOR"}, {eName:"TH",eText:"TIPO"}, {eName:"TH",eText:"SUBTOTAL"}, {eName:"TH",eText:"IMPUESTOS"}, {eName:"TH",eText:"TOTAL"}]}]},onlySATBody]})]);
            }
            if ((+jobj.numDiferentes)>0) {
                let differsBody={eName:"TBODY",eChilds:[]};
                for(let i=0; i<jobj.diferentes.length; i++) {
                    let row=jobj.diferentes[i];
                    differsBody.eChilds.push({eName:"TR",className:"csvRow",eChilds:[{eName:"TD",className:"hidden",eText:"SI"},{eName:"TD",className:"hidden",eText:"SI"},{eName:"TD",className:"maxfit",eText:row.uuid},{eName:"TD",eText:row.fecha},{eName:"TD",eText:row.remision},{eName:"TD",eText:row.receptor},{eName:"TD",eText:row.tipo},{eName:"TD",eText:row.subtotal},{eName:"TD",eText:row.impuestos},{eName:"TD",eText:row.total}]});
                    csvContent+="SI,SI,"+row.uuid+","+row.fecha+","+row.remision+","+row.receptor+","+row.tipo+","+row.subtotal+","+row.impuestos+","+row.total+"\n";
                }
                appendMessageElement(dra, [ecrea({eName:"H2",className:"centered marginHSp",eText:"En ambos con diferencias: "+jobj.numDiferentes+"/"+jobj.diferentes.length}), ecrea({eName:"TABLE",className:"maxfit centered screen",eChilds:[{eName:"THEAD",eChilds:[{eName:"TR",eChilds:[{eName:"TH",className:"hidden",eText:"Avance"},{eName:"TH",className:"hidden",eText:"SAT"},{eName:"TH",eText:"UUID"}, {eName:"TH",eText:"FECHA"}, {eName:"TH",eText:"REMISION"}, {eName:"TH",eText:"RECEPTOR"}, {eName:"TH",eText:"TIPO"}, {eName:"TH",eText:"SUBTOTAL"}, {eName:"TH",eText:"IMPUESTOS"}, {eName:"TH",eText:"TOTAL"}]}]},differsBody]})]);
            }
...
function adjustCalMonImgs(tgtWdgt) {
    ...
    let prevClass = "calendar_month_"+padDate(prevMon);
    let nextClass = "calendar_month_"+padDate(nextMon);

    let calMonPrev = document.getElementById("calendar_month_prev");
    let calMonNext = document.getElementById("calendar_month_next");
