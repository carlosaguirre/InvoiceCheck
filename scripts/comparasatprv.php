...
function loadCSV(name) {
  ...
  let tgtid="loadCSV";
  if (name==="comparaavanceprv") tgtid+="Avance";
  else if (name==="comparasatprv") tgtid+="SAT";
  else return;
  let tgt=ebyid(tgtid);
  let file=tgt.files[0];
  let url="consultas/comparaSAT.php";
  let parameters={accion:"altaprv",name:name,file:file};
    ...
    if (state===4) {
      if (status==200) {
        try {
          ...
          postService("consultas/Errores.php",{accion:"savelog",nombre:"comparasatprv",texto:text});
        }
...
function muestraDatos(tabla,recargar) {
  ...
  if (!tBody||recargar) {
    ...
    let parameters={tableid:tabla.id,tablename:tabla.getAttribute("name"),accion:"consultaprv",fieldnames:"uuid,fecha,tipoComprobante,rfcEmisor,rfcReceptor",finicio:aIni+"-"+mIni+"-"+dIni+" 00:00:00",ffin:aFin+"-"+mFin+"-"+dFin+" 23:59:59",empresa:empresa,tipo:tipo};
    if (!tBody) {
...
function comparaDatos() {
  ...
  let empresa=ebyid("empresa").value;
  let tipo=ebyid("tipo").value;
  let parameters={accion:"comparaAmbasPrv",finicio:aIni+"-"+mIni+"-"+dIni+" 00:00:00",ffin:aFin+"-"+mFin+"-"+dFin+" 23:59:59",empresa:empresa,tipo:tipo};
  overlayMessage("<p>OBTENIENDO DATOS...</p>","... PROCESANDO ...");
  let successFunc=function(params, resultText, state, status) {
    if (state===4) {
      if (status==200) {
        try {
          let jobj=JSON.parse(resultText);
...
function eliminaDatos() {
  ...
  let empresa=ebyid("empresa").value;
  let tipo=ebyid("tipo").value;
  let parameters={accion:"eliminaprv",finicio:aIni+"-"+mIni+"-"+dIni+" 00:00:00",ffin:aFin+"-"+mFin+"-"+dFin+" 23:59:59",empresa:empresa,tipo:tipo,tabla:tabla.getAttribute("name")};
  let successFunc=function(resultText, params, state, status) {
    if (state===4) {
      if (status==200) {
        try {
...
<?php
clog1seq(-1);
clog2end("scripts.comparasatprv");
