...
function muestraDatos(tabla,recargar) {
  ...
  if (!tBody||recargar) {
    let parameters={tableid:tabla.id,tablename:tabla.getAttribute("name"),accion:"consulta_c",fieldnames:"uuid,fecha,tipoComprobante,rfcEmisor,rfcReceptor"};
    if (!tBody) {
...
function comparaDatos() {
  ...
  let parameters={accion:"comparaAmbas_c",finicio:aIni+"-"+mIni+"-"+dIni+" 00:00:00",ffin:aFin+"-"+mFin+"-"+dFin+" 23:59:59",empresa:empresa};
