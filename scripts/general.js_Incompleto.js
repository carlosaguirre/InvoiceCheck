...
function toFormObject(name,value) {
    if (!prefix) prefix="";
    if (value instanceof File) {
        funclog("PARAM",name+"=file("+value.name+")");
    ...
    if (Array.isArray(value)||value instanceof FileList) {
        ...
        for (let i=0;i<value.length;i++) {
            let namei=""+i;
            if (name.length>0) namei=name+"["+namei+"]";
            let retVal=toFormObject(namei,value[i]);
            if (Array.isArray(retVal) && Array.isArray(retVal[0])) retArr.push(...retVal);
            else retArr.push(retVal);
        }
        return retArr;
    }
    if (typeof value==="object") {
        const retArr=[];
        for (let pk in value) if (value.hasOwnProperty(pk) && typeof value[pk]!=="function") {
            if (pk==="timeout") retArr.push(["timeout",value[pk]]);
            else {
                let namei=pk;
                if (name.length>0) namei=name+"["+namei+"]";
                else if (namei==="timeout")
                let retVal=toFormObject(namei,value[pk]);
                if (Array.isArray(retVal) && Array.isArray(retVal[0])) retArr.push(...retVal);
                else retArr.push(retVal);
            }
        }
    ...
    if (!name||name.length==0) name="parameter";
    funclog("PARAM",name+"="+value);
    return [name,value];
}
function postService(url, parameters, retFunc, errFunc, progressFunc) {
    ...
    let fd = new FormData();
    paramList=toFormObject(parameters);
    paramList.forEach(d=>{});
    if (parameters) {
                ...
                if(key==="timeout") {
                    ...
                    for (let i=0;i<pk.length;i++) {
                        if (pk[i] instanceof File) {
                            ...
                            if (typeof pk[i]==="object") pk[i]=JSON.stringify(pk[i]);
                    ...
                    for (let pp in pk) {
                        ...
                        if (ppVal instanceof File) {
                            ...
                            if (typeof ppVal==="object") ppVal=JSON.stringify(ppVal);
                    ...
                    if (txt.length>0) {
                        ...
                        if (jobj.result) {
                            ...
                            if (result==="refresh"||result==="reload") location.reload(true);
                                ...
                                jobj.params=params;
                                if (typeof rdyFunc==="function") rdyFunc(jobj,extra);
