...
var isProcessing=false; // Bandera para reconocer cuando se está esperando algo porque asi debe ser (respuesta del servidor, algun video o lectura de un documento que sea forzoso esperar, alguna otra acción que requiera esperar). Se debe implementar las funciones que controlen estas variables. Seguramente hay que ponerlas en un script aparte y a futuro se haga todo lo posible para que el cliente no tenga forma de checarlo
var do15MinReload=false; // Bandera para ignorar reload cuando pasen 15 minutos

// Clock Functionality
function startInterval() {
    let clock = ebyid('pie_clock');
    if (clock) {
        clock.initTime=now.getTime();
        //clock.onmouseover=clockTimerDisplay;
    } //else console.log("Error startInterval no clock!");
...
function updateTime() {
    ...
    nowMS += 1000;
    now.setTime(nowMS);
    let capUsr = ebyid("caption_usuario");
    let clock = ebyid('pie_clock');
    if (capUsr && clock) {
        if (!clock.initTime) {
            clock.initTime=now.getTime();
        ...
        const innerTimer=now.getTime()-clock.initTime;
        let tm = {dys:0,hrs:0,mns:0,scs:Math.floor(innerTimer/1000)};
        if (tm.scs>=60) {
            tm.mns=Math.floor(tm.scs/60);
            tm.scs%=60;
            if (tm.mns>=60) {
                tm.hrs=Math.floor(tm.mns/60);
                tm.mns%=60;
                if (tm.hrs>=24) {
                    tm.dys=Math.floor(tm.hrs/24);
                    tm.hrs%=24;
                }
            }
        }
        let ftm = "";
        if (tm.dys>0) ftm+=tm.dys+" days";
        if (tm.hrs>0) {
            if (ftm.length>0) ftm+=", ";
            ftm+=tm.hrs+" hrs";
        }
        ...
        //if (ftm.length>0) console.log("tick "+ftm);
        if (do15MinReload) if (tm.dys>0 || tm.hrs>0 || tm.mns>=15) doReload();
    } // else console.log("No clock or no user!");
}
function restartClockTimer() {
    let capUsr = ebyid("caption_usuario");
    let clock = ebyid('pie_clock');
    if (capUsr && clock) clock.initTime=now.getTime();
}
function formatTime(ms) {
    if (ms < 1000) return `${ms} milliseconds`;
    if (ms < 10000 && ms % 1000 !== 0) return `${(ms / 1000).toFixed(1)} seconds`;
...
//function clockTimerDisplay(evt) {
    //console.log("INI func clockTimerDisplay");
//    const tgt=evt.currentTarget;
    //funclog("INI","clockTimerDisplay"+((tgt && tgt.innerTimer)?" "+tgt.innerTimer:""));
//    if (tgt && tgt.innerTimer) tgt.title=tgt.innerTimer;
//}
...
function postService(url, parameters, retFunc, errFunc, progressFunc) {
    ...
    // OPEN wait
    funclog("OPEN","POST Service Request. url: "+url+", parameters: "+JSON.stringify(parameters,jsonCircularReplacer()));
    ...
    if (errFunc && typeof errFunc==="function") {
        xmlHttpPost.onerror = function(evt) {
            ...
            if (url!=="consultas/Logs.php") logService("postService ERROR with errFunc", {
                oldUrl:url,
                oldReadyState:evt.target.readyState, 
                oldStatus:evt.target.status, 
                oldParameters:parameters,
                xhpParameters:xmlHttpPost.parameters,
                oldResponseText:evt.target.responseText,
                xhpResponseText:xmlHttpPost.responseText});
        };
        xmlHttpPost.ontimeout = function(evt) {
            errFunc("Request Timed Out", xmlHttpPost.parameters, evt);
            if (url!=="consultas/Logs.php") logService("postService TIMEOUT with errFunc", {
                oldUrl:url,
                oldReadyState:evt.target.readyState, 
                oldStatus:evt.target.status, 
                oldParameters:parameters,
        ...
        xmlHttpPost.onerror = function(evt) {
            ...
            if (url!=="consultas/Logs.php") logService("postService ERROR", {
                oldUrl:url,
                oldReadyState:evt.target.readyState, 
                oldStatus:evt.target.status, 
                oldParameters:parameters,
        ...
        xmlHttpPost.ontimeout = function(evt) {
            ...
            if (url!=="consultas/Logs.php") logService("postService TIMEOUT", {
                oldUrl:url,
                oldReadyState:evt.target.readyState, 
                oldStatus:evt.target.status, 
                oldParameters:parameters,
    ...
    if (progressFunc && typeof progressFunc==="function") {
        xmlHttpPost.onprogress = function (evt) {
            progressFunc(xmlHttpPost.responseText, xmlHttpPost.parameters, xmlHttpPost.readyState, xmlHttpPost.status, evt);
        };
    } else if (parameters.hasProgress) {
        xmlHttpPost.onprogress = function(evt) {
            ...
            funclog("PROGRESS","postService. "+(progressFunc?(" ("+(typeof progressFunc)+")"):"")+": "+txt);
        }
    }
    xmlHttpPost.send(fd);
...
function logService(textMessage, data) {
    ...
    readyService(
        ...
        parameters,
        function(j,x,n) {
            console.log("LOGSERVICE RESULT:",j,"DATA:",x,"NEXT:",n);
        },
        function(m,r,x) {
            console.log("LOGSERVICE ERROR MESSAGE: ",m," RESULT DATA: ",r," INPUT DATA: ",x);
        }
    );
}
function sessionService(key,value) {
    readyService("consultas/Usuarios.php",{sessionKey: key, sessionValue: value, respType: "text"},(txt,extra)=>{console.log("SESSION RESULT: "+txt+" "+key+(txt=="SET"?"='"+value+"'":""));},false);
}
...
function prePostService(url, parameters, rdyFunc, errFunc, prgFunc) {
    const postErrFunc=getPostErrFunc(errFunc,{...parameters, errSetAt: 'prePostServiceINI'});
...
function getPostRetFunc(rdyFunc,errFunc,extra) {
    funclog("INI function getPostRetFunc extra: "+JSON.stringify(extra,jsonCircularReplacer()));
    return (responseText,params,readyState,hStatus,prgEvt)=>{
        doShowFuncLogs=true;
        let statusInfo=readyState+"/"+hStatus;
        /*if (prgEvt) {
            statusInfo+=" | Progress ";
            if (prgEvt.lengthComputable) { // useless unless streaming (knowing the full size of data in bytes apriori)
                const percentComplete = (prgEvt.loaded / prgEvt.total) * 100;
                statusInfo+=""+percentComplete.toFixed(2)+"%";
            } else {
                statusInfo+="unknown";
            }        
        }*/
        ...
        try {
            ...
            if (readyState===3 && !params.hasProgress && !extra.hasProgress) {
                funclog("END","RETFUNC: PROGRESS NOT VALID "+statusInfo);
                return false;
            }
            if (readyState===3 && extra.notProgressFunc) {
                funclog("END","RETFUNC: THIS IS NOT THE PROGRESS FUNC "+statusInfo);
                return false;
            }
            ...
            if (params.hasProgress) {
                if (xobj && xobj.inclusiveSeparator && xobj.inclusiveSeparator.length>0) {
                    xobj.prgCount++;
                    statusInfo+=" | #PRG="+xobj.prgCount;
                    //funclog("CHK","ORIGINAL RESPONSETEXT: "+originalResponseText);
                    responseText = originalResponseText.replaceAll(",\"inclusiveSeparator\":\""+xobj.inclusiveSeparator+"\"", "");
                }
                //funclog("CHK","RESPONSETEXT: "+responseText);
            }
            funclog("INI","RETFUNC: "+statusInfo+"\nPARAMS="+JSON.stringify(params,jsonCircularReplacer())/*+"\nXOBJ="+JSON.stringify(xobj,jsonCircularReplacer())+"\nEXTRA="+JSON.stringify(extra,jsonCircularReplacer())*/);
            //if ((readyState==3||readyState==4) && hStatus==200) {
                let responseList=false;
                if (xobj && (params.hasProgress || extra.hasProgress) && xobj.progressIndex!==undefined && xobj.progressIndex!==false) {
                    ...
                    if (xobj.inclusiveSeparator && xobj.inclusiveSeparator.length>0) {
                        ...
                        funclog("CHK","RETFUNC: lastSeparatorIndex="+lastSeparatorIndex);
                        if (lastSeparatorIndex>xobj.progressIndex) {
                            ...
                            funclog("CHK","RETFUNC: responseList="+JSON.stringify(responseList));
                            ...
                            responseText="";
                            funclog("CHK","RETFUNC: RESPONSETEXT TO EMPTY");
                        }
                    } else {
                        funclog("CHK","RETFUNC: NONE INCLUSIVE SEPARATOR");
                        ...
                        if (responseText.length>0) {
                            //console.log("READY IN ONCE: ",responseText);
                            responseList=[responseText];
                        }
                    }
                    funclog("CHK","RETFUNC: TEST IF EMPTY");
                    if (responseText.length==0) {
                ...
                else responseList=[responseText];

                //postService("consultas/Errores.php",{accion:"savelog",nombre:"getPostRetFunc",texto:"TEST RESULT "+statusInfo+"\nTEXT: "+responseText+"\nPARAMS: "+JSON.stringify(params,jsonCircularReplacer())});
                funclog("CHK","RETFUNC: DO RESPONSELIST: #"+responseList.length);
                if (responseList) {
                    responseList.forEach((txt,idx,arr)=>{
                        if (txt.length>0) {
                            if (params.respType && params.respType==="text") {
                                if (readyState==4 && rdyFunc && typeof rdyFunc==="function") {
                                    funclog("RdyFunc('"+txt+"')");
                                    //console.log("RdyFunc(txt): ",rdyFunc);
                                    rdyFunc(txt,extra);
                                }
                            } else {
                                let limpio = txt.replace(/<!--[\s\S]*?-->/g, '');
                                if (limpio.length>0) {
                                    ...
                                    if (jobj.result) {
                                        ...
                                        } else console.log("INI getPostRetFunc.result.responseBlock="+limpio);
                                    } else throw new Error("SIN RESULTADO");
                                } else console.log("EMPTY JSON OBJECT WITH COMMENTS");
                            }
            ...
            postService("consultas/Errores.php",{accion:"savelog",nombre:"getPostRetFunc",texto:messageError+" | "+statusInfo+"\nTEXT: "+responseText+"\nPARAMS: "+JSON.stringify(params,jsonCircularReplacer())});
            if (responseText.slice(0,16)==="<!-- REFRESH -->") {
                doReload(true);
            }
        } finally {
...
//    funclog("TAG", "AjaxPost sending:"+keys);
    ...
    */
    ...
    xmlHttpPost.onerror = function(evt) {
        ...
        if (url!=="consultas/Logs.php") logService("AjaxPost ERROR", {
            oldUrl:url,
            oldReadyState:evt.target.readyState, 
            oldStatus:evt.target.status, 
            oldParameters:parameters,
    ...
    xmlHttpPost.ontimeout = function(evt) {
        ...
        if (url!=="consultas/Logs.php") logService("AjaxPost TIMEOUT", {
            oldUrl:url,
            oldReadyState:evt.target.readyState, 
            oldStatus:evt.target.status, 
            oldParameters:parameters,
...
function fillSelector(selectorname, params, title, dialogId) {
    ...
    let waitImage="imagenes/icons/rollwait2.gif";
    if (_win_) waitImage=_win_;
    element.innerHTML = "<div id='waitRoll' class='centered'><img src='"+waitImage+"' class='hg100 padhtt'></div>";
...
function focusOnAutoFocus(parentElement) {
    ...
    if (elemList && elemList.length>0) {
        ...
        repeatFocus(elem,10);
        console.log("END function focusOnAutoFocus: "+getElIdf(elem));
        return true;
    }
...
function repeatFocus(elem,times) {
    if (times<=0) return;
    elem.focus();
    console.log("Repeat Focus "+times+": "+getElIdf(document.activeElement)+" vs "+getElIdf(elem,true));
    if (elem!==document.activeElement) setTimeout(repeatFocus, 20, elem, times-1);
    //else console.log("Active Elem = ",document.activeElement);
}
function getElIdf(elem, onlyId) {
    if (onlyId && elem.id) return "#"+elem.id;
    return elem.tagName+(elem.id?"#"+elem.id:(elem.name?"|"+elem.name:""))+(elem.type?"("+elem.type+")":"")+(elem.value?"="+elem.value:"");
}
/* **
...
function _overlayCheck(name,msg,title,dialogId,saveFunc) {
    ...
    appendMessageElement(element, msg, true); appendMessageElement(ttElement, title);
...
function overlayMessage(msg, title, dialogId, repeatAutoFocus) {
    ...
    ekfil(ttlElem);
    appendMessageElement(element,msg,true);
...
function getParagraphObject(text, extraClassName, replaceClassName) {
    ...
    if (replaceClassName) {
        ...
        if (!pgObj.className||pgObj.className.length==0) pgObj.className="padhtt";
        else pgObj.className+=" padhtt";
...
function getTotalHeight(elem) {
    const eoh=elem.offsetHeight;
    if (eoh<=0) return 0;
    const cst=window.getComputedStyle(elem);
    return eoh+parseFloat(cst.marginTop)+parseFloat(cst.marginBottom);
}
function adjustDialogBoxHeight(onlySet) {
    ...
    const crw = ebyid("close_row");
    const cba = ebyid("closeButtonArea");
    cladd(drar,"calc");
    const dblen = dbox.offsetHeight;
    const drlen = drar.offsetHeight;
    const xtlen = getTotalHeight(crw)+getTotalHeight(cba);
    const calcRLen = dblen - xtlen;
    let msg="";
    if (drlen>0) {
        if (calcRLen < drlen) {
...
function appendErrorMessageProperty(evt) { // properties: errorDetail, keepFunc=false
    funclog("INI","appendErrorMessageProperty");
    //console.log("INI function appendErrorMessageProperty",evt);
    const tgt=evt.target;
    const msg=tgt.errorDetail;
    const kpFnc=tgt.keepFunc;
    if (appendMessageElement(tgt,msg)) {
        if (!kpFnc) {
            //if (tgt.onclick) console.log(" ### ONCLICK=",tgt.onclick);
            //if (tgt.ondblclick) console.log(" ### ONDBLCLICK=",tgt.ondblclick);
            if (tgt.onclick===appendErrorMessageProperty) {
                //console.log(" ### REMOVED ONCLICK");
                //delete tgt.onclick;
                tgt.onclick=null;

            } else if (tgt.ondblclick===appendErrorMessageProperty) {
                //console.log(" ### REMOVED ONDBLCLICK");
                //delete tgt.ondblclick;
                tgt.ondblclick=null;
            }// else console.log(" ### FUNCTIONS click OR dblclick ARE NOT appendErrorMessageProperty");
        }// else console.log(" ### KEEP FUNC appendErrorMessageProperty");
    }
}
function appendMessageElement(element, message, padded) {
    let strElem=""; let strMsg="";
    try { strElem=JSON.stringify(element); } catch (error) { strElem=String(element); }
    try { strMsg=JSON.stringify(message); } catch (error) { strMsg=String(message); }
    funclog("INI","appendMessageElement Element='"+strElem+"', Message='"+strMsg+"'");
    try {
        if (!element||!message) {
        ...
        if (Array.isArray(message)) {
            //funclog("Array");
            let addSome=false;
            message.forEach(function(submessage) { if (appendMessageElement(element,submessage, padded)) addSome=true; });
            return addSome;
        } else if (typeof message==="string") {
            if (message.length>400) funclog("MSG: \'"+message.substr(0,200)+"...\n"+"..."+message.substr(-200)+"\'");
            else funclog("MSG: \'"+message+"\'");
            const strobj={eName:"DIV",eText:"."};
            if (padded===true) padded="padhtt"; // padt10,pad10,pad20
            if (typeof padded==="string") strobj.className=padded;
            const strdiv=ecrea(strobj);
            //const strdiv=ecrea({eName:"P",eText:"."});
...
function toggleCheck(elementId, result=null, depth) {
    if (typeof depth === 'undefined') depth=0;
    let element = ebyid(elementId);
    if (element) {
        if (result == null) result=!element.checked;
        element.checked=!!result;
    } else if (depth < 10) {
        let newDepth = depth+1;
        appendLog("RETRY | toggleCheck "+newDepth);
        setTimneout(function() {
            toggleCheck(elementId, result, newDepth);
        }, 20);
    } else {
        appendLog("ERROR | toggleCheck missing: element");
    }
}
...
function funclog(pref, txt, lvl) {
    if (doShowFuncLogs) { // doShowLogs=true|>0
        if (arguments.length>2) {
            if(!lvl) return; // falsy argument not undefined
            if (typeof lvl === 'number' && typeof doShowFuncLogs === 'number' && doShowFuncLogs<lvl) return; // number out of range
            // all other cases are valid
        }
        if (arguments.length==1) {
            txt=pref;
...
function calculaRangoSeleccionado(elemento, inilen) {
    ...
    if (!inilen && inilen!==0) inilen=endlen;
    ...
    if (resultado && resultado.length>0) { // && resultado.indexOf("|")>=0) {
        ...
        if (idAccion=="grupo") {
            if (keydown) {
                ...
                if (sIdx !== false) {
                    console.log("INI resultadoAvanzado: rarr grupo(down) sIdx="+sIdx,rarr);
                    fillValue("grupo_id", rarr[3*sIdx]);
                    fillValue("grupo_field", rarr[3*sIdx+1]);
                    fillValue("grupo_code", rarr[3*sIdx+2]);
                    toggleCheck("grupo_filtro1",rarr[3*sIdx+3]&&1);
                    toggleCheck("grupo_filtro2",rarr[3*sIdx+3]&&2);
                    toggleCheck("grupo_filtro3",rarr[3*sIdx+3]&&4);
                    fillInnerHtml("grupo_descripcion", rarr[3*sIdx+2]);
                    return true;
                }
                return false;
            } else {
                console.log("INI resultadoAvanzado: rarr grupo(up)",rarr);
                fillValue("grupo_id", rarr[0]);
                fillValue("grupo_field", rarr[1]);
                fillValue("grupo_code", rarr[2]);
                toggleCheck("grupo_filtro1",rarr[3]&&1);
                toggleCheck("grupo_filtro2",rarr[3]&&2);
                toggleCheck("grupo_filtro3",rarr[3]&&4);
                fillInnerHtml("grupo_descripcion", rarr[2]);
                return findEqualIndexInRows(rarr,4,1);
            }
        } else if (idAccion=="proveedor") {
            if (keydown) {
            ...
            let ksys=ebyid("user_syskey");
            let vsys=ebyid("user_sysval");
            if (ksys&&vsys) {
                vsys.value=rarr[5];
                ksys.checked=((+vsys.value)>0);
            }
            fillValue("user_obs", rarr[6]);
            updateProfilesPerUser(rarr[7],rarr[8]);
            return false;
        }
    } else {
        ...
        if (idAccion=="grupo") {
            ...
            fillValue("grupo_code", "");
            toggleCheck("grupo_filtro1",false);
            toggleCheck("grupo_filtro2",false);
            toggleCheck("grupo_filtro4",false);
            fillInnerHtml("grupo_descripcion", "");
        } else if (idAccion=="proveedor") {
...
var currLst=["MXN","USD","EUR"];
function currencySwitch(evt) {
    const tgt=evt.target;
    if (tgt) {
        if (!tgt.oldValue) tgt.oldValue=tgt.value;
        let cF=false;
        currLst.forEach((cV,cI)=>{
            if(!cF && tgt.value===cV){
                cF=true;
                tgt.value=currLst[cI<(currLst.length-1)?cI+1:0];
            }
        });
        if(!cF) tgt.value=currLst[0];
    }
}
...
function ignoreSpecialChars(event) {
    if (!event) {
        if (window.event) event=window.event;
        else return false;
    }
    ...
    if (                 keycode < 48 ||  // 48 – 57 (0 – 9)
       ( keycode > 57 && keycode < 65) || // 65 – 90 (A – Z)
       ( keycode > 90 && keycode < 97) || // 97 – 122 (a – z)
         keycode > 122 ) {
        // Restrict the special characters
        eventCancel(event);  
        return false;
...
function ignoreSpaces(evt) {
    return preventKeyCodes(evt, [32]);
}
function removeSpaces(evt) {
    const tgt=evt?evt.target:null;
    if (tgt) tgt.value = tgt.value.replace(/\s/g, "");
}
...
function copyContentToClipboardByClass(classname) {
    const elems = lbycn(classname);
    const message = getAllTextIn(elems, " ", 10, "\n");
...
function copyFieldToClipboard(elem) {
    try {
        if (elem.select) {
            elem.select();
            return document.execCommand('copy');
        } else {
            return copyTextToClipboard(elem.textContent);
        }
    } catch (err) {
...
function copyToClipboard(evt) {
    if(!evt) evt = window.event;
    let tgt = evt.target!=null ? evt.target: evt.srcElement;
    const result = copyFieldToClipboard(tgt);
    if (result===true) console.log("Text copied: '"+tgt.textContent+"'!");
    else if (result===false) console.log("Text NOT copied!");
    else console.log("Error while copying text: "+result);
}
function copyTextAreaElemToClipboard(textAreaElem, logDivElem, briefLogDivElem) {
    ...
    if (textAreaElem && textAreaElem.value.length>0) {
        const result = copyFieldToClipboard(textAreaElem);
        if (result===true) {
            logText = "La ruta fue copiada satisfactoriamente";
            briefLog = "COPIADO";
            success = true;
        } else if (result===false) {
            logText = "No se pudo copiar la ruta";
            briefLog = "NO COPIADO";
        } else {
            logText = 'Error al copiar texto: '+result;
...
function isValidFile(file,validType) {
    ...
    if (validType &&((Array.isArray(validType) && !validType.includes(type))||type!==validType))
        return "El archivo '"+name+"' no tiene el formato requerido ("+type+")";
    const size=+file.size;
    if (size>2097000)
        return "El archivo '"+name+"' excede el tamaño máximo permitido de 2MB ("+size+")";
    return true;
}
...
function htmlTableToArray(elem, hasHeaders, separator, options=false) {
    ...
    if (elem) {
        ...
        for (let child of elem.children) {
            ...
            if (child.tagName==="THEAD" && hasHeaders) {
                if (csv.length>0)
                    csv=[ ...csv, ...htmlTableToArray(child, true, separator, options)];
                else csv=htmlTableToArray(child, true, separator, options);
            }
            if (child.tagName==="TBODY") {
                if (csv.length>0)
                    csv=[ ...csv, ...htmlTableToArray(child, false, separator, options)];
                else csv=htmlTableToArray(child, false, separator, options);
            }
            if (child.tagName==="TR") {
                const row=[];
                let idx=0;
                for (let cell of child.children) {
                    if (["TD","TH"].includes(cell.tagName)) {
                        if (!options||options.includes(idx)) {
                            let data="";
                            ...
                            row.push(data);
                            if (cell.hasAttribute("value2")) {
                                data=cell.getAttribute("value2");
                                if (data.length>0) data='"'+data.replace(/(\r\n|\n|\r)/gm, '').replace(/(\s\s)/gm, ' ').replace(/"/g, '""')+'"';
                                row.push(data);
                            }
                        }
...
function arrayToCSVFile(arr,filename,addDate,nocheck,removeAfterClick=true) {
    ...
    if (arr.length>0) {
        ...
        link.click();
        if (removeAfterClick) document.body.removeChild(link);
        else return link;
        return true;
    }
    return false;
...
// SHORTCUTS
// toDo: replace doSome with function name that receives a function as argument
function mergeObjects(targetObj, sourceObj) {
    for (let key in sourceObj) {
        if (sourceObj.hasOwnProperty(key) && typeof sourceObj[key] !== "function") {
            targetObj[key] = sourceObj[key];
        }
    }
    return targetObj;
}
function fee(arrayLike, elemCallback, funcname) { // for each class element: callback(elem[,index,array])
    if (Array.from) {
...
function clfunc(elem,classname,funcname,params) {
    if (Array.isArray(elem) || elem instanceof NodeList || elem instanceof HTMLCollection) {
        let count=0;
        fee(elem, subelem=>{ count+=clfunc(subelem,classname,funcname,params); });
        return count;
    }
    if (typeof elem==="string") elem=ebyid(elem);
    let count=0;
    if (elem && elem.classList && classname) {
        if (!Array.isArray(classname)) classname=[classname];
        if (params && !Array.isArray(params)) params=[params];
        let neg=false;
        let sFuncname=false;
        if (funcname==="add" || funcname==="remove") { neg=(funcname==="add"); sFuncname=funcname; funcname="contains"; }
        fee(classname,cn=>{
...
function clset(elem,classname, boolval) { if (typeof boolval!=="boolean") return 0; return clfunc(elem,classname,"toggle",boolval); }
function clswt(elem,classname1, classname2) { return clfunc(elem,classname1,"remove") + clfunc(elem,classname2,"add"); }
...
function ekfil(elem, exceptionList) {
    ...
    if (elem) {
        if (typeof elem==="string") {
            ...
            elem.forEach(e=>ekfil(e,exceptionList));
            return;
        }
...
function getTextWidth(text, font) {
    ...
    if (!font) font="14px Tahoma, Arial, sans-serif";
    context.font=font;

    // Measure the text width
    let metrics=context.measureText(text);
    return metrics.width;
    // Example usage: getTextWidth("Hello, world!", "16px Arial")+"px";
}
