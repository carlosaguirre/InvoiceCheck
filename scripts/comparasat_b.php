...
function loadCSV(name) {
  ...
  let parameters={accion:"alta_b",name:name,file:file};
...
function muestraDatos(tabla,recargar) {
  ...
  if (!tBody||recargar) {
    let parameters={tableid:tabla.id,tablename:tabla.getAttribute("name"),accion:"consulta_b",fieldnames:"uuid,fecha,tipoComprobante"};
    if (!tBody) {
...
function comparaDatos() {
  ...
  let parameters={accion:"comparaAmbas_b",finicio:aIni+"-"+mIni+"-"+dIni+" 00:00:00",ffin:aFin+"-"+mFin+"-"+dFin+" 23:59:59"};
      ...
      if (status==200) {
        try {
          ...
          if (jobj.result) { // && jobj.result==="success"
            ...
            let csvContent="Avance,SAT,UUID,FECHA,TIPO\n";
            ...
            if ((+jobj.numSoloAvance)>0) {
                ...
                for(let i=0; i<jobj.soloAvance.length; i++) {
                    ...
                    onlyAvnBody.eChilds.push({eName:"TR",className:"csvRow",eChilds:[{eName:"TD",className:"hidden",eText:"SI"},{eName:"TD",className:"hidden",eText:"NO"},{eName:"TD",className:"maxfit",eText:row.uuid},{eName:"TD",eText:row.fecha},{eName:"TD",eText:row.tipo}]});
                    csvContent+="SI,NO,"+row.uuid+","+row.fecha+","+row.tipo+"\n";
                }
                appendMessageElement(dra, [ecrea({eName:"H2",className:"centered marginHSp",eText:"Sólo Avance: "+jobj.numSoloAvance+"/"+jobj.soloAvance.length}), ecrea({eName:"TABLE", className:"maxfit centered screen",eChilds:[{eName:"THEAD",eChilds:[{eName:"TR",className:"csvRow",eChilds:[{eName:"TH",className:"hidden",eText:"Avance"},{eName:"TH",className:"hidden",eText:"SAT"},{eName:"TH",eText:"UUID"}, {eName:"TH",eText:"FECHA"}, {eName:"TH",eText:"TIPO"}]}]},onlyAvnBody]})]);
            }
            if ((+jobj.numSoloSat)>0) {
                let onlySATBody={eName:"TBODY",eChilds:[]};
                for(let i=0; i<jobj.soloSat.length; i++) {
                    let row=jobj.soloSat[i];
                    onlySATBody.eChilds.push({eName:"TR",className:"csvRow",eChilds:[{eName:"TD",className:"hidden",eText:"NO"},{eName:"TD",className:"hidden",eText:"SI"},{eName:"TD",className:"maxfit",eText:row.uuid},{eName:"TD",eText:row.fecha},{eName:"TD",eText:row.tipo}]});
                    csvContent+="NO,SI,"+row.uuid+","+row.fecha+","+row.tipo+"\n";
                }
                appendMessageElement(dra, [ecrea({eName:"H2",className:"centered marginHSp",eText:"Sólo SAT: "+jobj.numSoloSat+"/"+jobj.soloSat.length}), ecrea({eName:"TABLE",className:"maxfit centered screen",eChilds:[{eName:"THEAD",eChilds:[{eName:"TR",eChilds:[{eName:"TH",className:"hidden",eText:"Avance"},{eName:"TH",className:"hidden",eText:"SAT"},{eName:"TH",eText:"UUID"}, {eName:"TH",eText:"FECHA"}, {eName:"TH",eText:"TIPO"}]}]},onlySATBody]})]);
            }
