...
var waitImgName="flying";
/* Navidad */waitImgName="reindeer"; // "santasleigh" // "snowman" // "xmastree"
// var _oldZIndex = 0;
    ...
    if (parameters) {
        if (typeof parameters==="object") {
            for (let key in parameters) if (parameters.hasOwnProperty(key) && typeof parameters[key] !== "function") {
                ...
                if (Array.isArray(pk) || pk instanceof FileList) {
        ...
        if (retFunc) {
            if (typeof retFunc==="function") {
                ...
                if (xmlHttpPost.status!=200&&retmsg.length==0) {
                    ...
                    console.log("CHK | statusText = "+retmsg);
                }
                retFunc(retmsg, parameters, xmlHttpPost.readyState, xmlHttpPost.status);
            } else console.log("CHK | retFunc: "+(typeof retFunc),retFunc);
        }
    };
...
function postService2(url, parameters, rdyFunc, errFunc, prgFunc) {
    //console.log("INI function postService2b url="+url+", parameters:",parameters);
    if (parameters) parameters.hasProgress=true;
    else parameters={hasProgress: true};
    const postErrFunc=getPostErrFunc(errFunc,parameters);
    let postRetFunc=null;
    let postPrgFunc=null;
    if (prgFunc) {
        postRetFunc=getPostRetFunc(rdyFunc, errFunc, {hasProgress: true, notProgressFunc: true, validEmptyClosure: true});
        postPrgFunc=getPostRetFunc(prgFunc, errFunc, {hasProgress: true, notReadyFunc: true});
    } else {
        postRetFunc=getPostRetFunc(rdyFunc, errFunc, {hasProgress: true, validEmptyClosure: true});
    }
    const xmlHttpPost=postService(url,parameters,postRetFunc,postErrFunc,postPrgFunc);
    if(parameters.inclusiveSeparator) xmlHttpPost.inclusiveSeparator=parameters.inclusiveSeparator;
    return xmlHttpPost;
}
function getPostRetFunc(readyFunc,exceptionFunc,extra) {
    ...
    return (responseText,params,readyState,hStatus)=>{
        ...
        try {
            if (readyState>4 || hStatus>200) throw new Error("SERVICIO NO DISPONIBLE");
            ...
            //if ((readyState==3||readyState==4) && hStatus==200) {
                ...
                if (xobj && xobj.progressIndex!==undefined && xobj.progressIndex!==false) {
                    ...
                    if (xobj.inclusiveSeparator && xobj.inclusiveSeparator.length>0) {
                        ...
                        if (responseText.length>0) responseList=[responseText];
                    }
                    ...
                    if (txt.length>0) {
                        ...
                        if (jobj.result) {
                            ...
                            if (result==="refresh"||result==="reload") location.reload(true);
                                ...
                                jobj.params=params;
                                if (typeof readyFunc==="function") readyFunc(jobj,extra);
                                else console.log("INI getPostRetFunc.readyFunc is not function: ",readyFunc);
                ...
                else if (readyState==4 && readyFunc && typeof readyFunc==="function" && extra.validEmptyClosure) {
                    //console.log("EMPTY LAST BLOCK");
                    if (xobj.lastJObj) extra.lastJObj=xobj.lastJObj;
                    readyFunc(null,extra);
                }
            //}
        } catch (err) {
            let messageError="";
            ...
            if (exceptionFunc) {
                if (typeof exceptionFunc==="function") exceptionFunc(messageError,responseText,extra);
                else console.log("INI getPostRetFunc.exceptionFunc is not function: ",exceptionFunc);
...
function fillSelector(selectorname, params, title, dialogId) {
    ...
    element.innerHTML = "<div id='waitRoll' class='centered'><img src='imagenes/icons/rollwait2.gif' class='hg100 padhtt'></div>";
...
function viewWaitBackdrop() {
    ...
    clearBackdrop();
    showBackdropChild({eName:"IMG",id:"waitBackdropImage",src:"imagenes/icons/"+waitImgName+".gif",style:"position:fixed;z-index:8898;top:0px;left:0px;display:none;"});
    resizeWaitBackdrop();
