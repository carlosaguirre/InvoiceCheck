...
function fillData($table, $row) {
    if (!isset($_GET["tabla"]) && !isset($_GET["datos"]))
        return "alert('".$table."[".$row["id"]."] = ".$row["codigo"]." | ".$row["razonSocial"]." | ".$row["rfc"]."')";
    return "fillValue('proveedor_id','".$row["id"]."');fillValue('proveedor_field','".$row["razonSocial"]."');fillValue('proveedor_code','".$row["codigo"]."');fillValue('proveedor_rfc','".$row["rfc"]."');fillValue('proveedor_status','".$row["status"]."');fillValue('proveedor_zona','".$row["zona"]."');fillValue('proveedor_cuenta','".$row["cuenta"]."',10);fillValue('proveedor_recibo','".$row["edocta"]."',10); fillDataCheck(); overlay();";
}
...
if ($obj->numrows > 0) {
    if (!isset($_GET["datos"])) {
        $prvSttArr = array( ""=>"", "registrado"=>"registrado", "activo"=>"activo", "vencido"=>"vencido" );
?>
      <input type="hidden" name="selectortablename" id="selectortablename" value="<?= $obj->tablename ?>">
      <input type="hidden" name="selectorname" id="selectorname" value="showProveedores">
      <table>
        <thead>
          <tr>
            <th>C&oacute;digo</th><th>Raz&oacute;n Social</th><th>RFC</th><th>Status</th>
          </tr>
          <tr>
            <th><input type="text" onkeyup="fillSelectorContents('filter')" name="codigo" class="longtext filter_box text_selection"></th>
            <th><input type="text" onkeyup="fillSelectorContents('filter')" name="razonSocial" class="longtext filter_box text_selection"></th>
            <th><input type="text" onkeyup="fillSelectorContents('filter')" name="rfc" class="longtext filter_box text_selection"></th>
            <th><select onchange="fillSelectorContents('exacto')" name="status" class="filter_box text_selection">
<?= getHtmlOptions($prvSttArr, "", 20) ?>
            </select></th>
          </tr>
        </thead>
...
?>
          <tr>
            <td ondblclick="<?= fillData('proveedor', $row); ?>"><?= $row['codigo'] ?></td>
            <td ondblclick="<?= fillData('proveedor', $row); ?>"><?= $row['razonSocial'] ?></td>
            <td ondblclick="<?= fillData('proveedor', $row); ?>" class="shrinkCol"><?= $row['rfc'] ?></td>
            <td ondblclick="<?= fillData('proveedor', $row); ?>" class="shrinkCol"><?= $row['status'] ?></td>
          </tr>
<?php
    }
?>
