...
if (isset($_POST["module"][0])) {
    switch($_POST["module"]) {
        case "NOCCP": // readyService(\"selectores/generico.php\", {module:\"NOCCP\",totRows:{$invObj->numrows}, rdyFnc:showOnReady, errFnc:showOnError}, showOnReady, showOnError);
            $result=["clase"=>"Facturas","pageno"=>"1","limit"=>"100","where"=>"f.tipoComprobante=\"i\" and f.metodoDePago=\"PPD\" and f.idReciboPago is null and f.statusn between 32 and 127 and (f.fechaPago is null or f.fechaPago>\"2018-09-01 00:00:00\") and p.status!=\"inactivo\"","fields"=>"f.codigoProveedor, p.razonSocial, p.rfc, f.version, f.ciclo, count(1) n","extra"=>"f inner join proveedores p on f.codigoProveedor=p.codigo","groupBy"=>"f.codigoProveedor,f.version,f.ciclo","msgId"=>"msg","msgAs"=>"textContent","dataId"=>"noccpbody","dataAs"=>"tbrow","rowClass"=>"bbtm1d","dataFields"=>["#","codigoProveedor","razonSocial","rfc","version","ciclo","n"],"footId"=>"noccpfoot","url"=>"selectores/generico.php"];
            $_POST+=$result;
        break;
    }
}
